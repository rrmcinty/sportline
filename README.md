...
## Mobile Dashboard (AWS CDK)

Access your recommendations from anywhere with the cloud-based mobile dashboard built with **AWS CDK** (TypeScript infrastructure as code):

```bash
# One-command setup (dev environment)
npm run all
...
This deploys a complete CDK stack with:
- **S3 bucket** with lifecycle rules for models, features, and recommendations
- **Update Lambda** (runs daily at 8am EST via EventBridge)
- **Dashboard Lambda** (serves mobile-optimized UI with Function URL)
- **IAM roles** with least-privilege permissions (auto-generated by CDK)

**Why CDK?** TypeScript everywhere - infrastructure, Lambdas, and frontend all in TypeScript with full type safety and IDE support.

**Cost: ~$0.02-0.50/month** (within AWS free tier)

After deployment, CDK outputs your dashboard URL:
```
SportlineStack-dev.DashboardUrl = https://abc123xyz.lambda-url.us-east-1.on.aws
```

Open on your phone and add to home screen for app-like experience.

See `infrastructure/README.md` for full CDK documentation.
