...
import {
  fetchOdds as fetchOddsNcaam,
  normalizeOdds as normalizeOddsNcaam,
} from "../espn/ncaam/odds.js";
import { fetchEvents as fetchEventsCfb } from "../espn/cfb/events.js";
import {
  fetchOdds as fetchOddsCfb,
  normalizeOdds as normalizeOddsCfb,
} from "../espn/cfb/odds.js";
import { fetchEvents as fetchEventsNfl } from "../espn/nfl/events.js";
import {
  fetchOdds as fetchOddsNfl,
  normalizeOdds as normalizeOddsNfl,
} from "../espn/nfl/odds.js";
import { fetchEvents as fetchEventsNba } from "../espn/nba/events.js";
import {
  fetchOdds as fetchOddsNba,
  normalizeOdds as normalizeOddsNba,
} from "../espn/nba/odds.js";
import {
  evaluateParlay,
  generateParlays,
  rankParlaysByEV,
...
import {
  getLatestBacktestForConfig,
  type BacktestResults,
} from "../model/backtest-storage.js";
import { getOptimalSeasons } from "../model/optimal-config.js";
import {
  logRecommendedBet,
  markBetsAsPlaced,
  type TrackedBet,
} from "../tracking/bet-logger.js";
import * as readline from "readline";
import { trainUnderdogModel } from "../underdog/underdog-train.js";
import {
  predictUnderdogs,
  displayUnderdogPredictions,
} from "../underdog/underdog-predict.js";
import {
  backtestUnderdogModel,
  compareUnderdogVsMainModel,
} from "../underdog/underdog-backtest.js";
 ...
 */
const NFL_SPREAD_ROI_BY_BUCKET: Record<
  string,
  { roi: number; winRate: number; count: number }
> = {
...
export async function cmdGamesFetch(sport: Sport, date: string): Promise<void> {
  try {
    ...
    for (const comp of competitions) {
      ...
      console.log(
        chalk.gray(`  ${comp.awayTeam.name} @ ${comp.homeTeam.name}`),
      );
      console.log(chalk.gray(`  Venue: ${comp.venue || "N/A"}`));
      console.log(
        chalk.gray(`  Date: ${new Date(comp.date).toLocaleString()}`),
      );
      console.log();
    }
    ...
    console.log(
      chalk.dim(
        `💡 Tip: Use ${chalk.white("odds --sport " + sport + " --event <eventId> --date <date>")} to see betting lines`,
      ),
    );
  } catch (error) {
 ...
 */
export async function cmdOddsImport(
  sport: Sport,
  eventId: string,
  date: string,
): Promise<void> {
  try {
    ...
    console.log(
      chalk.dim(
        `(Probabilities shown are "vig-free" - the bookmaker's edge has been removed)\n`,
      ),
    );
    ...
    if (moneylines.length > 0) {
      console.log(
        chalk.bold("💰 Moneylines") +
          chalk.dim(" (bet on who wins straight-up):"),
      );
      for (const leg of moneylines) {
        const color = leg.impliedProbability > 0.5 ? chalk.green : chalk.yellow;
        console.log(
          `  ${color(leg.description)} → ${(leg.impliedProbability * 100).toFixed(1)}% chance`,
        );
      }
      console.log();
    }
    ...
    if (spreads.length > 0) {
      console.log(
        chalk.bold("📊 Spreads") + chalk.dim(" (bet on margin of victory):"),
      );
      for (const leg of spreads) {
        console.log(
          `  ${chalk.cyan(leg.description)} → ${(leg.impliedProbability * 100).toFixed(1)}% chance`,
        );
      }
      console.log();
    }
    ...
    if (totals.length > 0) {
      console.log(
        chalk.bold("🎲 Totals") +
          chalk.dim(" (bet on combined score over/under):"),
      );
      for (const leg of totals) {
        console.log(
          `  ${chalk.magenta(leg.description)} → ${(leg.impliedProbability * 100).toFixed(1)}% chance`,
        );
      }
      console.log();
    }
    ...
    console.log(
      chalk.dim(
        `💡 Tip: Lower vig = better value. Moneylines typically have the lowest bookmaker edge.`,
      ),
    );
  } catch (error) {
...
export async function cmdBets(
  ...
  try {
    ...
    console.log(
      chalk.dim(
        `(Market probabilities shown - vig removed | Model predictions in brackets)\n`,
      ),
    );
    ...
    try {
      ...
      if (spreadModelProbs && !spreadModelProbs.has(eventId)) {
        console.log(
          chalk.dim(
            `⚠️  No spread model prediction for this event (${spreadModelProbs.size} events have predictions)\n`,
          ),
        );
      }
      if (totalModelProbs && !totalModelProbs.has(eventId)) {
        console.log(
          chalk.dim(
            `⚠️  No totals model prediction for this event (${totalModelProbs.size} events have predictions)\n`,
          ),
        );
      }
    } catch (err) {
      ...
      for (const leg of mLegs) {
        ...
        if (market === "moneyline" && modelProbs) {
          ...
          if (homeProb !== undefined) {
            const prob = leg.description.includes(
              comp.homeTeam.abbreviation || comp.homeTeam.name,
            )
              ? homeProb
              : 1 - homeProb;
          ...
          if (overProb !== undefined) {
            const prob = leg.description.startsWith("Over")
              ? overProb
              : 1 - overProb;
            modelPrediction = ` [Model: ${(prob * 100).toFixed(1)}%]`;
          }
        ...
        const profitDisplay =
          profit > 0
            ? ` | Profit: ${chalk.green("$" + profit.toFixed(2))}`
            : "";
    ...
    if (best) {
      const prefix =
        best.ev >= 0
          ? chalk.green.bold("✨ Potential +EV")
          : chalk.dim("Least negative EV");
      console.log(
        prefix +
          chalk.dim(
            `: ${best.legs[0].description} (EV $${best.ev.toFixed(2)}, ROI ${best.roi.toFixed(2)}%)`,
          ),
      );
    }
    console.log(
...
async function getBacktestStats(
  ...
  try {
    ...
    const backtest = await getLatestBacktestForConfig(
      sport,
      market,
      optimalSeasons,
    );
    ...
    const totalWins = backtest.calibration.reduce((sum, bin) => {
      const binWinRate =
        bin.roi > 0 ? 0.5 + bin.roi / 200 : 0.5 + bin.roi / 200;
...
export async function cmdRecommend(
  ...
  try {
    ...
    const sportsToCheck: Sport[] = sports || [
      "ncaam",
      "cfb",
      "nfl",
      "nba",
    ...
    const sportDisplay = sports
      ? sports.join(", ").toUpperCase()
      : "ALL SPORTS";
    console.log(
      chalk.bold.cyan(
    ...
    const pendingBets = trackedData.bets.filter(
      (b) => b.actuallyBet && b.status === "pending",
    );
    ...
    for (const b of pendingBets) {
      ...
      const totalStake =
        placements.reduce((sum, p) => sum + (p.stake || 0), 0) || b.stake || 0;
    ...
    if (pendingBets.length > 0) {
      ...
      for (const bet of pendingBets) {
        ...
        const totalStake =
          placements.reduce((sum, p) => sum + (p.stake || 0), 0) ||
          bet.stake ||
          0;
        ...
        console.log(
          chalk.green(
            `  ✓ ${bet.matchup} - ${bet.pick.split("[")[0].trim()} [${bet.sport.toUpperCase()}] - $${totalStake.toFixed(2)} - ${dateStr}`,
          ),
        );
      }
      console.log();
      ...
      for (const d of dates) {
        try {
          ...
          allCompetitions.push(
            ...competitions.map((c: Competition) => ({ ...c, sport })),
          );
        } catch (err) {
    ...
    console.log(
      chalk.gray(
        `Found ${allCompetitions.length} games across ${sportsToCheck.length} sport(s) and ${days} day(s)\n`,
      ),
    );
    ...
    for (const comp of allCompetitions) {
      try {
        ...
        const gameRow = db
          .prepare(`SELECT id FROM games WHERE espn_event_id = ?`)
          .get(comp.eventId) as { id: number } | undefined;
        if (gameRow) {
          ...
          const insertOdds = db.prepare(
            `INSERT INTO odds (game_id, provider, market, line, price_home, price_away, price_over, price_under, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          );
    ...
    const eventIdToMatchup = new Map<
      string,
      { away: string; home: string; sport: Sport; date: string }
    >();
    ...
    for (const sport of sportsToCheck) {
      for (const d of dates) {
        try {
          ...
          const daySpreadProbs = await getHomeSpreadCoverProbabilities(
            sport,
            d,
          );
    ...
    for (const comp of allCompetitions) {
      ...
      try {
        ...
        const includeDogsEnv =
          process.env.SPORTLINE_INCLUDE_DOGS === "1" || false;
        const includeDogs = includeDogsEnv || includeDogsFlag;
        const EXTREME_UNDERDOG_THRESHOLD = 0.1; // <10% model probability
        if (!includeDogs) {
          legs = legs.filter((leg: BetLeg) => {
            ...
            if (["moneyline", "total"].includes(leg.market)) {
              if (
                ...
                leg.impliedProbability < EXTREME_UNDERDOG_THRESHOLD
              ) {
        ...
        if (favoritesOnly) {
          legs = legs.filter((leg: BetLeg) => {
            ...
            const baseProb =
              leg.marketImpliedProbability ?? leg.impliedProbability;
    ...
    console.log(
      chalk.gray(
        `Found ${allLegs.length} betting opportunities across ${allCompetitions.length} games`,
      ),
    );
    if (
      modelProbs.size > 0 ||
      spreadModelProbs.size > 0 ||
      totalModelProbs.size > 0
    ) {
      ...
      console.log(
        chalk.green.dim(
          `Model probabilities applied to ${markets.join(" and ")}`,
        ),
      );
    } else {
      console.log(
        chalk.dim("Using vig-free market probabilities (no model override)"),
      );
    }
    ...
    if (divergenceThreshold > 0) {
      ...
      allLegs.splice(
        ...
        ...allLegs.filter((leg) => {
          ...
          const divergence =
            Math.abs(leg.impliedProbability - leg.marketImpliedProbability) *
            100;
      ...
      console.log(
        chalk.cyan(
          `🔍 Divergence filter: showing only ${allLegs.length}/${beforeFilter} bets where |model - market| ≥ ${divergenceThreshold}%`,
        ),
      );
      if (allLegs.length === 0) {
        console.log(
    ...
    if (allLegs.length > MAX_LEGS_FOR_PARLAYS) {
      console.log(
        chalk.yellow(
          `⚠️  Too many betting opportunities (${allLegs.length}). Filtering to top ${MAX_LEGS_FOR_PARLAYS} by EV for parlay generation.`,
        ),
      );
      ...
      allLegs.splice(
        0,
        allLegs.length,
        ...singleBetEVs.slice(0, MAX_LEGS_FOR_PARLAYS).map((x) => x.leg),
      );
      console.log(
        chalk.gray(
          `Filtered to ${allLegs.length} legs for parlay combinations\n`,
        ),
      );
    }
    ...
    if (totalModelProbs && totalModelProbs.size > 0) {
      ...
      console.log(
        chalk.magenta.dim(
          `Totals model distribution: n=${n}, mean=${(mean * 100).toFixed(1)}%, std=${(std * 100).toFixed(1)}%, range=[${(min * 100).toFixed(1)}%, ${(max * 100).toFixed(1)}%], >70%: ${high}, <30%: ${low}`,
        ),
      );
    }
    console.log(
      chalk.dim(`Calculating expected value (EV) with fair probabilities...\n`),
    );

    // Categorize bets by confidence tier based on backtest results
    function getConfidenceTier(prob: number): {
      tier: string;
      emoji: string;
    ...
    if (minLegs === 1 || allLegs.length > 0) {
      console.log(
        chalk.bold.green(`📊 BEST SINGLE BETS`) +
          chalk.dim(` (ranked by confidence + EV)\n`),
      );
      console.log(
        chalk.dim(
          `What's EV? It's the average $ you'd win/lose per $${stake} bet over many tries.`,
        ),
      );
      console.log(
        chalk.dim(
          `Positive EV = good bet. Negative EV = bookmaker has the edge.\n`,
        ),
      );

      const singleBets = allLegs.map((leg) =>
        evaluateParlay({ legs: [leg], stake }),
      );
      ...
      for (const bet of singleBets) {
        ...
        const sportName: Sport =
          (matchupInfo?.sport?.toLowerCase() as Sport) || "nba";
        ...
        if (backtestStats && backtestStats.hasData) {
          const roi =
            backtestStats.roi !== null && backtestStats.roi !== undefined
              ? backtestStats.roi
              : -999;
      ...
      for (const bet of backtestedSingles) {
        ...
        if (marketType === "moneyline" && leg.odds >= 150 && leg.odds <= 300) {
          ...
          if (underdogData && underdogData.roi > 0) {
            const preference =
              UNDERDOG_HOME_AWAY_PREFERENCE[sportName] || "home";
            const isHomeUnderdog = leg.description.includes(
              matchupInfo?.home || "___NOMATCH___",
            );
            const isAwayUnderdog = leg.description.includes(
              matchupInfo?.away || "___NOMATCH___",
            );

            // Check if this underdog matches the sport's preferred location
            const matchesPreference =
              (preference === "home" && isHomeUnderdog) ||
        ...
        if (marketType === "spread" && sportName === "nfl") {
          ...
          const spreadProfile = checkNFLSpreadProfile(
            sportName,
            marketType,
            spreadLine,
            homeATSRecord,
      ...
      for (const { bet, roi: binRoi } of rankedSingles.slice(0, topN)) {
        ...
        const datePart = matchupInfo?.date
          ? new Date(matchupInfo.date).toISOString().split("T")[0]
        ...
        let bin = "50-60%";
        if (displayProb < 0.1) bin = "0-10%";
        else if (displayProb < 0.2) bin = "10-20%";
        else if (displayProb < 0.3) bin = "20-30%";
        else if (displayProb < 0.4) bin = "30-40%";
        else if (displayProb < 0.5) bin = "40-50%";
        else if (displayProb < 0.6) bin = "50-60%";
        else if (displayProb < 0.7) bin = "60-70%";
        else if (displayProb < 0.8) bin = "70-80%";
        else if (displayProb < 0.9) bin = "80-90%";
        else bin = "90-100%";
        ...
        await logRecommendedBet({
          ...
          eventId: leg.eventId,
          matchup: matchupInfo
            ? `${matchupInfo.away} @ ${matchupInfo.home}`
            : "",
      ...
      console.log(
        chalk.dim(
          `Confidence distribution: 🏆 ${tiers.ELITE} Elite | ⭐ ${tiers.HIGH} High | 📊 ${tiers.MEDIUM} Medium | ⚠️ ${tiers["COIN FLIP"]} Coin Flip\n`,
        ),
      );
      ...
      for (let i = 0; i < topRankedSingles.length; i++) {
        ...
        const matchupInfo = eventIdToMatchup.get(leg.eventId);
        if (matchupInfo && matchupInfo.sport)
          sportName = matchupInfo.sport.toLowerCase() as Sport;
        ...
        const backtestStats = await getBacktestStats(
          sportName,
          marketType as "moneyline" | "spread" | "total",
          displayProb,
        );
        let marketStats = {
          winRate: "N/A",
        ...
        if (backtestStats && backtestStats.hasData) {
          ...
          marketStats = {
            winRate: backtestStats.winRate
              ...
              : "N/A",
            roi:
              backtestStats.roi !== null
                ? `${roiSign}${backtestStats.roi.toFixed(1)}%`
                : "N/A",
        ...
        if (matchup) {
          ...
          matchupDisplay = chalk.dim(
            `${matchup.away} @ ${matchup.home} - ${dateStr}`,
          );
        }
        ...
        let confidenceLabel = "";
        if (displayProb >= 0.8 || displayProb <= 0.2) {
          confidenceLabel = `${(displayProb * 100).toFixed(0)}% probability`;
        } else if (displayProb >= 0.7 || displayProb <= 0.3) {
          confidenceLabel = `${(displayProb * 100).toFixed(0)}% probability`;
        } else if (displayProb >= 0.6 || displayProb <= 0.4) {
          confidenceLabel = `${(displayProb * 100).toFixed(0)}% probability`;
        } else {
        ...
        let titleLine =
          titlePrefix +
          chalk.bold(`${i + 1}. ${tier.emoji} ${cleanDescription}`) +
          chalk.dim(` [${confidenceLabel}]`);
            ...
            const sideAbbrev =
              leg.team === "home"
                ...
                : leg.team === "away"
                  ? matchup.away
                  : undefined;
            placedKey = sideAbbrev
              ? `${matchup.sport}:${leg.eventId}:${leg.market}:${sideAbbrev}`
              : "";
          }
          if (placedKey && placedLookup.has(placedKey)) {
            const placed = placedLookup.get(placedKey)!;
            const placedOddsDisplay =
              placed.odds >= 0 ? `+${placed.odds}` : `${placed.odds}`;
            console.log(
              chalk.yellow(
                `   📌 Already placed: $${placed.stake.toFixed(2)} at ${placedOddsDisplay}`,
              ),
            );
          }
        }
        ...
        if (underdogInfo?.isProfitableUnderdog) {
          console.log(
            chalk.magenta(
              `   💎 Profitable underdog profile: +${underdogInfo.roi.toFixed(1)}% historical ROI in ${underdogInfo.sport.toUpperCase()} ${UNDERDOG_ROI_BY_SPORT[underdogInfo.sport].bucket}`,
            ),
          );
        }
        ...
        if (spreadInfo?.isProfitableSpread) {
          console.log(
            chalk.green(
              `   🏈 Profitable NFL spread profile: +${spreadInfo.roi.toFixed(1)}% historical ROI in ${spreadInfo.bucket} confidence bucket (${spreadInfo.winRate.toFixed(1)}% win rate)`,
            ),
          );
        }
        ...
        console.log(
          chalk.dim(
            `   Market: ${marketType === "moneyline" ? "Moneyline (win outright)" : marketType === "spread" ? "Point Spread" : "Total Points"}`,
          ),
        );
        ...
        if (leg.currentOdds !== undefined) {
          ...
          const oddsChangeStr =
            oddsChange > 0
              ? chalk.green(`+${oddsChange}`)
              : chalk.red(`${oddsChange}`);
          console.log(
            `   Odds: ${chalk.cyan(openingOddsStr)} (opening) → ${chalk.yellow(currentOddsStr)} ${oddsChangeStr}`,
          );
        } else {
        ...
        console.log(
          `   If you win: ${chalk.green("$" + bet.payout.toFixed(2) + " total")} ${chalk.dim("($" + potentialProfit.toFixed(2) + " profit)")}`,
        );
        console.log(
          `   Win chance: ${chalk.cyan((displayProb * 100).toFixed(1) + "%")}${isModelProb ? chalk.dim(" (model)") : ""}`,
        );
        const smallSample =
          sampleSize > 0 && sampleSize < MIN_SAMPLE_FOR_STRONG;

        if (marketStats.label) {
          const gamesInfo = marketStats.gamesLabel
            ? chalk.dim(` | ${marketStats.gamesLabel}`)
            : "";
          const seasonsInfo = marketStats.seasonsLabel
            ? chalk.dim(` | seasons ${marketStats.seasonsLabel}`)
            : "";
          const sampleNote = smallSample
            ? chalk.yellow.dim(" | small sample")
            : "";
          console.log(
            chalk.dim(
              `   Historical: ${marketStats.winRate} win rate, ${marketStats.roi} ROI - ${marketStats.label}${gamesInfo}${seasonsInfo}${sampleNote}`,
            ),
          );
        }
        console.log(
          `   Expected value: ${evColor(evSign + "$" + bet.ev.toFixed(2))} ${chalk.dim("per $10 bet")}${isModelProb ? chalk.dim(" (model)") : ""}`,
        );
        if (marketStats.winRate !== "N/A" && marketStats.roi !== "N/A") {
          ...
          if (smallSample) {
            console.log(
              chalk.yellow(
                `   ⚠️ Historical sample too small for confidence (${sampleSize} games).`,
              ),
            );
          } else if (roiNum >= 10) {
            console.log(
              chalk.green.bold(
                `   ${tier.emoji} Strong historical performance: ${marketStats.winRate} win rate, ${marketStats.roi} ROI in this range!`,
              ),
            );
          } else if (roiNum >= 0) {
            console.log(
              chalk.green(
                `   ${tier.emoji} Profitable historically: ${marketStats.winRate} win rate, ${marketStats.roi} ROI in this range.`,
              ),
            );
          } else if (roiNum >= -10) {
            console.log(
              chalk.yellow(
                `   ⚠️ Bookmaker has edge: ${marketStats.winRate} win rate, ${marketStats.roi} ROI historically.`,
              ),
            );
          } else {
            console.log(
              chalk.red(
                `   ❌ Poor value: ${marketStats.winRate} win rate, ${marketStats.roi} ROI historically.`,
              ),
            );
          }
        }
        if (bet.ev >= 0) {
          console.log(
            chalk.green.bold(`   ✨ This bet has positive expected value!`),
          );
        }
        console.log();
      }
      ...
      if (
        bestEV >= 0 &&
        bestTier &&
        (bestTier.tier === "ELITE" || bestTier.tier === "HIGH")
      ) {
        console.log(
          chalk.green.bold(
            `🎯 RECOMMENDATION: ${bestTier.emoji} ${bestTier.tier} confidence bet with +EV! Backtests show ${bestTier.historicalWinRate} success rate.`,
          ),
        );
      } else if (bestEV >= 0) {
        console.log(
          chalk.green.bold(
            `🎯 RECOMMENDATION: This bet has +EV, but lower confidence. Consider smaller stake.`,
          ),
        );
      } else if (
        bestTier &&
        ...
        console.log(
          chalk.yellow(
            `💡 INSIGHT: ${bestTier.tier} confidence bets available, but market is efficient (-EV). Wait for better opportunities.`,
          ),
        );
      } else if (bestEV > -0.5) {
        console.log(
          chalk.yellow(
            `💡 INSIGHT: Moneylines have the lowest bookmaker edge (~${Math.abs(bestEV * 10).toFixed(0)}%). Still negative EV though.`,
          ),
        );
      } else {
        console.log(
          chalk.yellow(
            `⚠️  INSIGHT: High bookmaker edge today. All bets lose ${Math.abs(rankedSingles[0]?.roi || 0).toFixed(1)}% on average.`,
          ),
        );
      }
      console.log();
      ...
      if (interactive && loggedBetIds.length > 0) {
        ...
        console.log(
          chalk.dim(`Enter numbers (e.g., "1,3,5"), "all", or "none":\n`),
        );
        ...
        if (answer && answer !== "none") {
          ...
          if (selectedIndices.length > 0) {
            ...
            const stakeAnswer = await new Promise<string>((resolve) => {
              ...
              rl2.question(
                chalk.cyan(
                  `Stake amount per bet ($). You can enter one value (e.g., 10) or comma-separated values matching your selection (e.g., 10,25,10): `,
                ),
                (ans) => {
                  rl2.close();
                  resolve(ans.trim());
            ...
            console.log(
              chalk.dim(
                `Run ${chalk.white("sportline results")} to check outcomes later\n`,
              ),
            );
          }
        } else {
        ...
        console.log(
          chalk.dim(
            `   Run with ${chalk.white("--interactive")} to mark which bets you actually placed\n`,
          ),
        );
      }
    }
    ...
    if (!includeParlays) {
      ...
      console.log(
        chalk.dim(
          `    Run with ${chalk.white("--include-parlays")} to see parlay options.\n`,
        ),
      );
    ...
    console.log(
      chalk.bold.blue(`🎰 PARLAY COMBINATIONS`) +
        chalk.dim(` (${minLegs}-${maxLegs} legs)\n`),
    );
    console.log(
      chalk.dim(
        `Parlays = betting multiple outcomes together. All must win to cash out.`,
      ),
    );
    console.log(
      chalk.dim(
        `Higher payout but lower win probability. Bookmaker edge compounds!\n`,
      ),
    );
    ...
    const backtestedLegs = allLegs.filter((leg) => {
      const isNFLTotal =
        leg.market === "total" && leg.description.includes("[NFL]");
      const isNBATotal =
        leg.market === "total" && leg.description.includes("[NBA]");
      const isNCAAMTotal =
        leg.market === "total" && leg.description.includes("[NCAAM]");
      return (
        leg.market === "moneyline" || isNFLTotal || isNBATotal || isNCAAMTotal
      );
    });
    console.log(
      chalk.dim(
        `Using ${backtestedLegs.length} backtested legs for parlay generation (moneyline all sports + NFL/NBA/NCAAM totals)\n`,
      ),
    );

    const parlaySpecs = generateParlays(
      backtestedLegs,
      minLegs,
      maxLegs,
      stake,
    ...
    if (positiveEV.length > 0) {
      console.log(
        chalk.green.bold(
          `✅ Found ${positiveEV.length} parlay(s) with POSITIVE expected value!\n`,
        ),
      );
      ...
      console.log(
        chalk.dim(
          `   Bookmaker's edge on these odds means expected losses on all combos.\n`,
        ),
      );
      console.log(
        chalk.gray(
          `Showing top ${Math.min(topN, ranked.length)} by EV (least bad):\n`,
        ),
      );
    ...
    console.log(
      chalk.dim(
        `  • Spread = bet on margin of victory (e.g., -5.5 means win by 6+)`,
      ),
    );
    console.log(
      chalk.dim(`  • Total = bet on combined score over/under a number`),
    );
    console.log(
      chalk.dim(`  • Parlay = multiple bets combined (all must win)`),
    );
    console.log(
      chalk.dim(`  • EV = expected value (average profit/loss per bet)`),
    );
    console.log(
      chalk.dim(
        `  • ROI = return on investment (EV as a percentage of stake)\n`,
      ),
    );
  } catch (error) {
 ...
 */
function printParlay(
  rank: number,
  parlay: ParlayResult,
  isPositiveEV: boolean,
  eventIdToMatchup: Map<
    string,
    { away: string; home: string; sport: Sport; date: string }
  >,
): void {
  ...
  console.log(
    `   Win chance: ${chalk.yellow((parlay.probability * 100).toFixed(2) + "%")} ${chalk.dim("(all legs must win)")}`,
  );
  console.log(
    `   Potential payout: ${chalk.green("$" + parlay.payout.toFixed(2))} ${chalk.dim("(profit: $" + parlay.profit.toFixed(2) + ")")}`,
  );
  console.log(
    `   Expected value: ${evColor(evSign + "$" + parlay.ev.toFixed(2))} ${evColor("(" + evSign + parlay.roi.toFixed(1) + "%)")}`,
  );

  if (isPositiveEV) {
    console.log(
      chalk.green.bold(
        `   ✨ POSITIVE EV - mathematically profitable over time!`,
      ),
    );
  } else {
    console.log(
      chalk.dim(
        `   📉 Bookmaker edge: lose ${Math.abs(parlay.roi).toFixed(1)}% on average`,
      ),
    );
  }
...
export async function cmdResults(): Promise<void> {
  const { loadTrackedBets, updateBetResults } =
    await import("../tracking/bet-logger.js");
  ...
  for (const bet of pendingBets) {
    ...
    if (game && (game.home_score > 0 || game.away_score > 0)) {
      ...
      if (bet.actuallyBet) {
        ...
        console.log(
          chalk.dim(`   Final: ${game.away_score}-${game.home_score}`),
        );
        if (bet.result) {
          const profitColor =
            bet.result.actualProfit! >= 0 ? chalk.green : chalk.red;
          console.log(
            profitColor(`   Profit: $${bet.result.actualProfit!.toFixed(2)}`),
          );
        }
        console.log();
      }
  ...
  if (updated === 0) {
    console.log(
      chalk.yellow(
        `No games have finished yet (${pendingBets.length} pending)`,
      ),
    );
  } else {
    ...
    console.log(
      chalk.dim(
        `Run ${chalk.white("sportline stats")} to see your overall performance\n`,
      ),
    );
  }
}
...
export async function cmdStats(): Promise<void> {
  const { getBetStats, loadTrackedBets } =
    await import("../tracking/bet-logger.js");
  ...
  if (stats.allRecommended.won + stats.allRecommended.lost > 0) {
    const winRate =
      (stats.allRecommended.won /
        (stats.allRecommended.won + stats.allRecommended.lost)) *
      100;
  ...
  const modelBetsWithResults = data.bets.filter(
    (b) => b.recommended && b.status !== "pending",
  );
  if (modelBetsWithResults.length > 0) {
    console.log(chalk.bold("Model Performance by Confidence Bin:"));
    // Group by sport first
    const modelSportBinStats: Record<
      string,
      Map<string, { won: number; lost: number; profit: number; staked: number }>
    > = {};
    ...
    for (const sport of Object.keys(modelSportBinStats).sort()) {
      ...
      for (const [bin, stat] of Array.from(binStats.entries()).sort()) {
        ...
        console.log(
          chalk.dim(
            `    ${bin}: ${stat.won}W-${stat.lost}L (${winRate.toFixed(0)}%)`,
          ),
        );
      }
    }
  ...
  if (stats.actuallyBet.count === 0) {
    ...
    console.log(
      chalk.dim(
        `  Run ${chalk.white("sportline recommend --interactive")} to track bets\n`,
      ),
    );
    return;
  }
  ...
  console.log(
    chalk.cyan(`  Total Staked: $${stats.actuallyBet.totalStaked.toFixed(2)}`),
  );

  const profitColor =
    stats.actuallyBet.totalProfit >= 0 ? chalk.green : chalk.red;
  console.log(
    profitColor(`  Total Profit: $${stats.actuallyBet.totalProfit.toFixed(2)}`),
  );

  if (stats.actuallyBet.won + stats.actuallyBet.lost > 0) {
    const winRate =
      (stats.actuallyBet.won /
        (stats.actuallyBet.won + stats.actuallyBet.lost)) *
      100;
    console.log(chalk.cyan(`  Win Rate: ${winRate.toFixed(1)}%`));

    const roiColor =
      stats.actuallyBet.roi >= 0 ? chalk.green.bold : chalk.red.bold;
    const roiSign = stats.actuallyBet.roi >= 0 ? "+" : "";
    console.log(
      roiColor(`  ROI: ${roiSign}${stats.actuallyBet.roi.toFixed(1)}%`),
    );
  }
  console.log();

  // Breakdown by bin
  const betsWithResults = data.bets.filter(
    (b) => b.actuallyBet && b.status !== "pending",
  );
  if (betsWithResults.length > 0) {
    ...
    const binStats = new Map<
      string,
      { won: number; lost: number; profit: number; staked: number }
    >();
    ...
    for (const [bin, stat] of Array.from(binStats.entries()).sort()) {
      ...
      console.log(
        chalk.dim(
          `  ${bin}: ${stat.won}W-${stat.lost}L (${winRate.toFixed(0)}%) - ${roiColor(roiSign + roi.toFixed(1) + "% ROI")}`,
        ),
      );
    }
    console.log();
    ...
    for (const bet of betsWithResults) {
      ...
      if (!sportBinStats[sport][bin])
        sportBinStats[sport][bin] = { won: 0, lost: 0, profit: 0, staked: 0 };
...
export async function cmdUnderdogPredict(
  ...
  try {
    const predictions = await predictUnderdogs(
      sport as any,
      date,
      minOdds,
      maxOdds,
...
export async function cmdUnderdogCompare(
  sport: string,
  seasons: number[],
): Promise<void> {
  try {
...
export async function cmdNFLSpreadTrain(seasons: number[]): Promise<void> {
  try {
    const { trainNFLSpreadModel } =
      await import("../spreads/nfl/nfl-spread-train.js");
...
export async function cmdNFLSpreadBacktest(seasons: number[]): Promise<void> {
  try {
    const { backtestNFLSpreadModel } =
      await import("../spreads/nfl/nfl-spread-backtest.js");
...
export async function cmdNFLSpreadAnalyze(
  seasons: number[],
  buckets?: string[],
): Promise<void> {
  try {
    const { analyzeNFLSpreadTraits } =
      await import("../spreads/nfl/nfl-spread-analyze.js");
