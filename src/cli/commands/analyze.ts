...
export async function analyze(options: AnalyzeOptions): Promise<void> {
  ...
  console.log(
    `   Model confidence: ${(filterConfig.minModelConfidence * 100).toFixed(0)}% - ${(filterConfig.maxModelConfidence * 100).toFixed(0)}%`,
  );
  console.log(`   Allowed months: ${filterConfig.allowedMonths.join(', ')}`);
  console.log(
    `   Allowed days: ${filterConfig.allowedDaysOfWeek.map((d) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')}`,
  );
  ...
  console.log(
    `   Prob diff range: ${(filterConfig.minImpliedProbDiff * 100).toFixed(1)}% to ${(filterConfig.maxImpliedProbDiff * 100).toFixed(1)}%`,
  );
  ...
  const filterResult = applySituationalFilters(allBetsForFiltering, dataset, filterConfig);
  ...
  if (filterResult.filteredRecommendations.length > 0) {
    ...
    const filteredTotalProfit = filteredProfitabilityAnalysis.allBets.reduce(
      (sum, b) => sum + b.betOutcome.profit,
      0,
    );
    const filteredTotalStaked = filteredProfitabilityAnalysis.allBets.reduce(
      (sum, b) => sum + b.betOutcome.betAmount,
      0,
    );
    const filteredROI = filteredTotalStaked > 0 ? filteredTotalProfit / filteredTotalStaked : 0;
    const filteredWinRate =
      filteredProfitabilityAnalysis.allBets.length > 0
        ? filteredProfitabilityAnalysis.profitableBets.length /
          filteredProfitabilityAnalysis.allBets.length
        : 0;
    ...
    const unfilteredTotalProfit = profitabilityAnalysis.allBets.reduce(
      (sum, b) => sum + b.betOutcome.profit,
      0,
    );
    const unfilteredTotalStaked = profitabilityAnalysis.allBets.reduce(
      (sum, b) => sum + b.betOutcome.betAmount,
      0,
    );
    const unfilteredROI =
      unfilteredTotalStaked > 0 ? unfilteredTotalProfit / unfilteredTotalStaked : 0;

    console.log(`\n📊 Comparison:`);
    console.log(`Unfiltered ROI: ${(unfilteredROI * 100).toFixed(2)}%`);
    console.log(`Filtered ROI: ${(filteredROI * 100).toFixed(2)}%`);
    console.log(
      `Improvement: ${((filteredROI - unfilteredROI) * 100).toFixed(2)} percentage points`,
    );

    if (filteredROI > unfilteredROI) {
      console.log(
        `✅ Filter improved ROI by ${((filteredROI - unfilteredROI) * 100).toFixed(2)} percentage points!`,
      );
    } else {
      console.log(
        `❌ Filter reduced ROI by ${((unfilteredROI - filteredROI) * 100).toFixed(2)} percentage points.`,
      );
    }
  }
  ...
  const baselineROI =
    profitabilityAnalysis.allBets.reduce((sum, b) => sum + b.betOutcome.profit, 0) /
  ...
  for (const { name, config: testConfig } of filterConfigs) {
    ...
    if (testFilterResult.filteredRecommendations.length > 0) {
      ...
      const testTotalStaked = testAnalysis.allBets.reduce(
        (sum, b) => sum + b.betOutcome.betAmount,
        0,
      );
      const testROI = testTotalStaked > 0 ? testTotalProfit / testTotalStaked : 0;
      const testWinRate =
        testAnalysis.allBets.length > 0
          ? testAnalysis.profitableBets.length / testAnalysis.allBets.length
          : 0;
      ...
      console.log(
        `${name.padEnd(14)} | ${' '.repeat(4)} | ${' '.repeat(4)} | ${' '.repeat(7)} | ${' '.repeat(7)} | No bets`,
      );
    }
  }
  ...
  for (const { name, config: testConfig } of filterConfigs.slice(1)) {
    // Skip 'No Filter'
    const testFilterResult = applySituationalFilters(allBetsForFiltering, dataset, testConfig);
