...
import { listAvailableHistoricalData, loadHistoricalData, getBestConfidenceBucket } from '../../lib/analysis/historicalDataManager.js';
...
export function createHistoricalStatusCommand(): Command {
  return new Command('historical-status')
    ...
    .action(async () => {
      ...
      console.log(`✅ Found historical data for ${availableSports.length} sports: ${availableSports.join(', ')}\n`);
      ...
      for (const sport of availableSports) {
        ...
        if (bestBucket) {
          console.log(`   🎯 Best Bucket: ${bestBucket.bucket}% confidence (+${(bestBucket.roi * 100).toFixed(1)}% ROI)`);
        }
        ...
        Object.entries(data.modelConfidenceBuckets)
          ...
          .forEach(([bucket, bucketData]) => {
            ...
            console.log(`     ${roiColor} ${bucket.padEnd(6)}: ${(bucketData.roi * 100).toFixed(1).padStart(6)}% ROI | ${(bucketData.winRate * 100).toFixed(1).padStart(5)}% win | ${bucketData.sampleSize.toString().padStart(3)} bets`);
          });
      ...
      for (const sport of availableSports) {
        ...
        if (bestBucket && bestBucket.roi > 0) {
          console.log(`✅ ${sport.toUpperCase()}: Target ${bestBucket.bucket}% confidence bucket (+${(bestBucket.roi * 100).toFixed(1)}% ROI)`);
        } else {
