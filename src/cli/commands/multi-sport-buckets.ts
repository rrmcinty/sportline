  ...
  return new Command('multi-sport-buckets')
    ...
    .action(async () => {
      ...
      const allBuckets: SportBucket[] = [
        ...
        { sport: 'NFL', bucket: '50-60%', count: 22, winRate: 81.8, roi: 37.6, estimatedBetsPerYear: 7, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NFL', bucket: '90-100%', count: 2, winRate: 50.0, roi: 100.0, estimatedBetsPerYear: 1, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NFL', bucket: '20-30%', count: 29, winRate: 48.3, roi: -2.0, estimatedBetsPerYear: 10, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        
        // NBA (Ultra-Optimized Model) - NEARLY BREAK-EVEN
        { sport: 'NBA', bucket: '60-70%', count: 368, winRate: 56.0, roi: -3.8, estimatedBetsPerYear: 46, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NBA', bucket: '70-80%', count: 66, winRate: 50.0, roi: -26.6, estimatedBetsPerYear: 8, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        
        // NCAAM (Ultra-Optimized Model) - HIGH VOLUME
        { sport: 'NCAAM', bucket: '80-90%', count: 593, winRate: 64.8, roi: -6.6, estimatedBetsPerYear: 74, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NCAAM', bucket: '50-60%', count: 480, winRate: 64.0, roi: -7.7, estimatedBetsPerYear: 60, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NCAAM', bucket: '0-10%', count: 385, winRate: 61.6, roi: -13.5, estimatedBetsPerYear: 48, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        
        // NHL (Ultra-Optimized Model) - STILL STRUGGLING
        { sport: 'NHL', bucket: '40-50%', count: 106, winRate: 56.6, roi: -7.3, estimatedBetsPerYear: 35, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NHL', bucket: '90-100%', count: 5, winRate: 60.0, roi: -11.1, estimatedBetsPerYear: 2, volumeScore: 0, roiScore: 0, combinedScore: 0 },
        { sport: 'NHL', bucket: '60-70%', count: 86, winRate: 55.8, roi: -12.3, estimatedBetsPerYear: 29, volumeScore: 0, roiScore: 0, combinedScore: 0 },
      ];
      ...
      sortedBuckets.forEach((bucket, index) => {
        ...
        console.log(`${index + 1}. ${profitableEmoji} ${volumeEmoji} ${bucket.sport} ${bucket.bucket} Confidence`);
        console.log(`   Games: ${bucket.count} (~${bucket.estimatedBetsPerYear}/year) | Win Rate: ${bucket.winRate.toFixed(1)}% | ROI: ${bucket.roi > 0 ? '+' : ''}${bucket.roi.toFixed(1)}%`);
      ...
      if (profitableBuckets.length > 0) {
        ...
        profitableBuckets.forEach((bucket, index) => {
          ...
          console.log(`   📊 Performance: ${bucket.winRate.toFixed(1)}% win rate (${bucket.count} games)`);
          ...
          console.log(`   📝 Strategy: Target ${bucket.sport} games with ${bucket.bucket} model confidence`);
          console.log('');
        });
        ...
        profitableBuckets.forEach((bucket, index) => {
          ...
          console.log(`   ${index + 1}. ${bucket.sport} ${bucket.bucket}: ~${bucket.estimatedBetsPerYear} bets/year × +${bucket.roi.toFixed(1)}% = +$${annualProfit.toFixed(0)}/year`);
        });
        
        console.log(`\n   🎯 TOTAL: ~${totalAnnualBets} bets/year with +$${totalAnnualProfit.toFixed(0)} annual profit`);
        console.log(`   📊 Average ROI: +${(totalAnnualProfit / (totalAnnualBets * 100) * 100).toFixed(1)}%`);
      ...
      const nearBreakEven = sortedBuckets.filter(b => b.roi > -10 && b.roi <= 0 && b.estimatedBetsPerYear >= 10);
      
      nearBreakEven.slice(0, 5).forEach((bucket, index) => {
        const oddsNeeded = Math.abs(bucket.roi) + 3; // How much better odds needed for +3% ROI
        console.log(`${index + 1}. ${bucket.sport} ${bucket.bucket}: ${bucket.roi.toFixed(1)}% ROI (~${bucket.estimatedBetsPerYear}/year)`);
