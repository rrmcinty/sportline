...
export function createOddsImpactCommand(): Command {
  return new Command('odds-impact')
    ...
    .action(async () => {
      ...
      const scenarios = [
        {
          description: 'Current Average Odds',
          improvement: 0,
          targetROI: currentBucket.currentROI,
        },
      ...
      scenarios.forEach((scenario, index) => {
        ...
        console.log(
          `   Annual Profit: ${annualProfit > 0 ? '+' : ''}$${annualProfit.toFixed(0)} (${currentBucket.estimatedBetsPerYear} bets × $100)`,
        );
