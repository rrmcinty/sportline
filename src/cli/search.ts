...
export async function cmdSearchTeam(
  ...
  try {
    ...
    for (const date of dates) {
      try {
        ...
        for (const comp of competitions) {
          const homeMatch =
            comp.homeTeam.name.toLowerCase().includes(query) ||
            (comp.homeTeam.abbreviation?.toLowerCase().includes(query) ??
              false);
          const awayMatch =
            comp.awayTeam.name.toLowerCase().includes(query) ||
            (comp.awayTeam.abbreviation?.toLowerCase().includes(query) ??
              false);
    ...
    if (allMatches.length === 0) {
      console.log(
        chalk.yellow(
          `No games found for "${teamQuery}" in the next ${days} days`,
        ),
      );
      console.log(chalk.dim(`\n💡 Tips:`));
      console.log(
        chalk.dim(
          `  • Try a shorter team name (e.g., "UNC" instead of "North Carolina")`,
        ),
      );
    ...
    for (const match of allMatches) {
      ...
      console.log(
        chalk.cyan(`  📊 View bets: `) +
          chalk.white(
            `sportline bets --event ${comp.eventId} --sport ${sport} --date ${match.date} --stake 10`,
          ),
      );
      console.log();
    ...
    console.log(
      chalk.white(
        `   sportline recommend --sport ${sport} --min-legs 1 --stake 10 --days ${days}`,
      ),
    );
  } catch (err) {
