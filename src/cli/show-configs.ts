...
export function cmdShowOptimalConfigs(): void {
  ...
  for (const sport of sports) {
    ...
    for (const market of markets) {
      ...
      const roiColor = config.expectedROI >= 5 ? chalk.green 
                     : config.expectedROI >= 0 ? chalk.yellow
                     : chalk.red;
      
      const status = config.expectedROI >= 5 ? "✅ RECOMMENDED"
                   : config.expectedROI >= 0 ? "⚠️  MARGINAL"
                   : "❌ AVOID";

      console.log(`  ${market.toUpperCase()}:`);
      console.log(`    Seasons: ${config.seasons.join(", ")}`);
      console.log(`    Expected ROI: ${roiColor(config.expectedROI >= 0 ? '+' : '')}${roiColor(config.expectedROI.toFixed(2) + '%')} | ECE: ${config.expectedECE.toFixed(2)}%`);
  ...
  console.log("  • NBA moneyline/spreads: Use 2 seasons (recent meta critical)");
  ...
  console.log("  • College sports generally prefer recent data (roster turnover)");
  console.log("  • Totals models generally benefit from more historical data\n");
  
  console.log(chalk.dim("To train with optimal config: sportline model train --sport <sport> --markets <market>"));
  console.log(chalk.dim("To override:                   sportline model train --sport <sport> --season 2024,2025 --markets <market>\n"));
}
