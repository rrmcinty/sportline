 ...
 */
function getSeasonDateRange(sport, season) {
    if (sport === "ncaam") {
 ...
 */
export async function cmdDataIngest(sport, season, fromDate, toDate) {
    try {
        ...
        for (const dateStr of dates) {
            try {
                ...
                for (const comp of competitions) {
                    // Upsert teams
                    const homeTeamResult = upsertTeam.get(sport, comp.homeTeam.id, comp.homeTeam.name, comp.homeTeam.abbreviation || null);
                    const awayTeamResult = upsertTeam.get(sport, comp.awayTeam.id, comp.awayTeam.name, comp.awayTeam.abbreviation || null);
                    ...
                    if ((sport === "ncaam" || sport === "nba") && comp.boxScore) {
                        ...
                        const homeStats = comp.boxScore.home;
                        const awayStats = comp.boxScore.away;
              ...
              `);
                    ...
                    // Insert game
                    insertGame.run(comp.eventId, sport, comp.date, season, homeTeamResult.id, awayTeamResult.id, comp.homeScore ?? null, comp.awayScore ?? null, comp.venue || null, comp.status);
                    totalGames++;
                    // Fetch and insert odds
                    try {
                        const oddsEntries = await fetchOdds(comp.eventId);
                        const gameRow = getGameByEventId.get(comp.eventId);
                        if (gameRow && oddsEntries.length > 0) {
                            for (const entry of oddsEntries) {
                                // Moneyline
                                if (entry.homeTeamOdds.moneyLine &&
                                    entry.awayTeamOdds.moneyLine) {
                                    insertOdds.run(gameRow.id, entry.provider.name, "moneyline", null, entry.homeTeamOdds.moneyLine, entry.awayTeamOdds.moneyLine, null, null, new Date().toISOString());
                                    totalOdds++;
                                }
                                // Spread
                                if (entry.spread !== undefined &&
                                    ...
                                    insertOdds.run(gameRow.id, entry.provider.name, "spread", entry.spread, entry.homeTeamOdds.spreadOdds, entry.awayTeamOdds.spreadOdds, null, null, new Date().toISOString());
                                    totalOdds++;
                                }
                                // Total
                                if (entry.overUnder !== undefined &&
                                    entry.overOdds &&
                                    entry.underOdds) {
                                    insertOdds.run(gameRow.id, entry.provider.name, "total", entry.overUnder, null, null, entry.overOdds, entry.underOdds, new Date().toISOString());
                                    totalOdds++;
                                }
                            }
                        }
 ...
 */
export async function cmdOddsRefresh(sports) {
    initDb();
    const db = getDb();
            ...
            try {
                ...
                if (oddsEntries.length > 0) {
                    for (const entry of oddsEntries) {
                        ...
                        if (entry.homeTeamOdds?.moneyLine &&
                            entry.awayTeamOdds?.moneyLine) {
                            insertOdds.run(game.id, entry.provider?.name || "Unknown", "moneyline", null, entry.homeTeamOdds.moneyLine, entry.awayTeamOdds.moneyLine, null, null, timestamp);
                            totalOdds++;
                        }
                        // Spread
                        if (entry.spread !== undefined &&
                            entry.homeTeamOdds?.spreadOdds &&
                            entry.awayTeamOdds?.spreadOdds) {
                            insertOdds.run(game.id, entry.provider?.name || "Unknown", "spread", entry.spread, entry.homeTeamOdds.spreadOdds, entry.awayTeamOdds.spreadOdds, null, null, timestamp);
                            totalOdds++;
                        }
                        // Total
                        if (entry.overUnder !== undefined &&
                            entry.overOdds &&
                            entry.underOdds) {
                            insertOdds.run(game.id, entry.provider?.name || "Unknown", "total", entry.overUnder, null, null, entry.overOdds, entry.underOdds, timestamp);
                            totalOdds++;
                        }
                    }
                }
