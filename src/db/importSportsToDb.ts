...
if (!sportArg || !Object.keys(SUPPORTED_SPORTS).includes(sportArg)) {
  console.error(
    `Invalid or missing sport argument. Supported sports: ${Object.keys(SUPPORTED_SPORTS).join(', ')}`,
  );
...
function importTeams() {
  ...
  for (const t of teams) {
    db.prepare(
      `INSERT OR IGNORE INTO teams (id, sport, name, abbreviation, display_name, short_display_name) VALUES (?, ?, ?, ?, ?, ?);`,
    ).run(
      t.teamId,
      sport,
      t.teamName,
...
function importGames() {
  ...
  for (const g of games) {
    ...
    const homeExists = db
      .prepare('SELECT 1 FROM teams WHERE id = ? AND sport = ?')
      .get(g.homeTeamId, sport);
    const awayExists = db
      .prepare('SELECT 1 FROM teams WHERE id = ? AND sport = ?')
      .get(g.awayTeamId, sport);
    ...
    db.prepare(
      `INSERT OR IGNORE INTO games (id, sport, date, season, home_team_id, away_team_id, home_score, away_score, venue, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
    ).run(
      g.id,
      sport,
      g.date,
...
function importOdds() {
  ...
  for (const o of odds) {
    ...
    if (!gameExists) {
      fs.appendFileSync(
        missingOddsGamesLog,
        `Odds for game ${o.eventId} (market: ${market}) missing game in games table.\n`,
      );
      continue;
    }
    ...
    db.prepare(
      `INSERT OR REPLACE INTO odds (game_id, provider, market, line, price_home, price_away, price_over, price_under, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`,
    ).run(
      o.eventId,
      o.provider,
      market,
...
function importGameStats() {
  ...
  const statConfig = SPORT_STAT_CONFIGS[sportType as keyof typeof SPORT_STAT_CONFIGS] || {
    combinedMetrics: {},
  };
  ...
  for (const s of stats) {
    if (!s.stats || !Array.isArray(s.stats) || s.stats.length === 0) {
      fs.appendFileSync(
        missingStatsLog,
        `game_id: ${s.game_id}, team_id: ${s.team_id} missing or empty stats, skipping.\n`,
      );
      continue;
    }
    ...
    if (!gameExists || !teamExists) {
      fs.appendFileSync(
        missingStatsLog,
        `Missing reference: game_id: ${s.game_id} exists: ${!!gameExists}, team_id: ${s.team_id} exists: ${!!teamExists}\n`,
      );
      continue;
    }
    for (const stat of s.stats) {
      ...
      if (!metricName) {
        fs.appendFileSync(
          missingStatsLog,
          `game_id: ${s.game_id}, team_id: ${s.team_id} missing metric_name and abbreviation, skipping stat: ${JSON.stringify(stat)}\n`,
        );
        continue;
      }
      ...
      if (
        metricName in statConfig.combinedMetrics &&
        typeof stat.value === 'string' &&
        stat.value.includes('-')
      ) {
        const [made, attempted] = stat.value.split('-').map(Number);
        const [madeName, attemptedName] = (
          statConfig.combinedMetrics as Record<string, [string, string]>
        )[metricName];
        if (!isNaN(made)) {
          db.prepare(
            `INSERT INTO game_stats (game_id, team_id, sport, season, metric_name, metric_value) VALUES (?, ?, ?, ?, ?, ?);`,
          ).run(s.game_id, s.team_id, sport, season, madeName, made);
          count++;
        }
        if (!isNaN(attempted)) {
          db.prepare(
            `INSERT INTO game_stats (game_id, team_id, sport, season, metric_name, metric_value) VALUES (?, ?, ?, ?, ?, ?);`,
          ).run(s.game_id, s.team_id, sport, season, attemptedName, attempted);
          count++;
        }
      ...
      db.prepare(
        `INSERT INTO game_stats (game_id, team_id, sport, season, metric_name, metric_value) VALUES (?, ?, ?, ?, ?, ?);`,
      ).run(s.game_id, s.team_id, sport, season, metricName, stat.value);
      count++;
    }
  }
    ...
    for (const cat of s.stats.categories) {
      ...
      for (const stat of cat.stats) {
        ...
        if (!metricName) {
          fs.appendFileSync(
            missingStatsLog,
            `teamId: ${s.teamId}, category: ${category} missing metric_name and abbreviation, skipping stat: ${JSON.stringify(stat)}\n`,
          );
          continue;
        }
        db.prepare(
          `INSERT INTO season_stats (team_id, sport, season, category, metric_name, metric_abbr, metric_value) VALUES (?, ?, ?, ?, ?, ?, ?);`,
        ).run(s.teamId, sport, season, category, metricName, stat.abbreviation ?? null, stat.value);
        count++;
      }
    }
  }
...
function main() {
  try {
    ...
    importSeasonStats();
    console.log(
      `✅ Import from JSON files complete for ${sportConfig.name} (${sport.toUpperCase()}) season ${season}.`,
    );
  } catch (error) {
