/**
 * ESPN NBA odds fetcher + normalizer
 */
...
interface ESPNOddsItem {
  ...
  homeTeamOdds?: { 
    ...
    close?: { moneyLine?: { american?: string }; spread?: { american?: string } };
  };
  awayTeamOdds?: { 
    favorite?: boolean; 
    underdog?: boolean; 
  ...
  close?: { over?: { american?: string }; under?: { american?: string }; total?: { american?: string } };
}
...
export async function fetchOdds(eventId: string): Promise<OddsEntry[]> {
  ...
  if (cached) {
    console.log(`✓ Cache hit for NBA odds on event ${eventId}`);
    return cached;
  }

  console.log(`Fetching NBA odds for event ${eventId}...`);
  ...
  const oddsEntries: OddsEntry[] = data.items.map(item => {
    ...
    const openSpread = item.spread; // Spread line typically doesn't change much
