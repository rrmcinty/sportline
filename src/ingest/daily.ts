  ...
  try {
    ...
    for (const comp of competitions) {
      try {
        ...
        const hadScoresBefore = db
          .prepare(
            `SELECT home_score, away_score FROM games WHERE espn_event_id = ?`,
          )
          .get(comp.eventId) as
        ...
        const result = getGameId.get(comp.eventId) as
          | { id: number }
          | undefined;
        ...
        if (result) {
          ...
          if (!hadScoresBefore) {
            ...
            hadScoresBefore.home_score === null &&
            comp.homeScore !== null
          ) {
            stats.gamesUpdated++;
          }
    ...
    stats.errors++;
    console.error(
      chalk.red(`  Error fetching ${sport} events for ${date}: ${err}`),
    );
  }
}
...
async function ingestSport(
  ...
  if (!latestDate) {
    console.log(
      chalk.yellow(
        `  No existing data for ${sport}, starting from 30 days ago`,
      ),
    );
    ...
    console.log(
      chalk.dim(
        `  Checking ${dates.length} dates (${startDate} to ${endDate})...`,
      ),
    );
    ...
    console.log(
      chalk.dim(
        `  Checking ${dates.length} dates (${startDate} to ${endDate}) to catch game updates...`,
      ),
    );
  ...
  console.log(
    chalk.green(
      `  ✓ ${sport.toUpperCase()}: ${stats.gamesAdded} new, ${stats.gamesUpdated} updated, ${stats.oddsAdded} odds added`,
    ),
  );
  if (stats.errors > 0) {
