...
async function asyncPool(
  poolLimit: number,
  array: any[],
  iteratorFn: (item: any, idx: number, arr: any[]) => Promise<any>,
) {
  const ret: any[] = [];
...
async function processAllTeams(season: number, league: LeagueConfig) {
  ...
  console.log(
    `Processing ${league.name.toUpperCase()} teams for stats, schedules, odds, and game stats...`,
  );
  ...
  await asyncPool(5, teamsToProcess, async (team: any) => {
    ...
    await asyncPool(5, trimmedSchedule, async (game: any) => {
      ...
      if (!oddsSeen.has(game.id) && Array.isArray(oddsArr)) {
        for (const odds of oddsArr) {
          ...
          // Moneyline market
          if (
            (odds.homeTeamOdds?.moneyLine !== undefined && odds.homeTeamOdds?.moneyLine !== null) ||
    ...
    console.log(
      `Processed team ${team.teamName} (${team.teamId}) - Games: ${trimmedSchedule.length}`,
    );
  });
  ...
  fs.appendFileSync(gameStatsFile, '\n]\n');
  console.log(
    `Finished processing ${teamsToProcess.length} teams, ${totalGames} games, ${totalOdds} odds, and ${totalGameStats} game stats. Output written to ${outDir}`,
  );
}
...
async function main() {
  try {
    ...
    if (!leagueArg || !LEAGUES[leagueArg]) {
      console.error(
        `Invalid or missing league. Supported leagues: ${Object.keys(LEAGUES).join(', ')}`,
      );
    ...
    console.log(
      `Starting ingestion for ${league.name.toUpperCase()} (${league.sport}) season ${season}`,
    );
