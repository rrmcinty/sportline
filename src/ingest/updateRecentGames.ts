...
async function updateRecentGames(daysBack: number = 1, daysForward: number = 7) {
  const now = new Date();
  ...
  for (let i = 0; i < gamesToUpdate.length; i += BATCH_SIZE) {
    ...
    console.log(
      `📦 Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(gamesToUpdate.length / BATCH_SIZE)} (${batch.length} games)...`,
    );
