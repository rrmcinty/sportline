...
const FALLBACK_HISTORICAL_DATA = {
  modelConfidence: {
    low: { roi: -0.2, winRate: 0.45, sampleSize: 100, description: 'Low Confidence (Fallback)' },
    medium: {
      roi: 0.01,
      winRate: 0.5,
      sampleSize: 200,
      description: 'Medium Confidence (Fallback)',
    },
...
export function getHistoricalContext(
  ...
  if (sportHistoricalData) {
    ...
    const confidenceLevel =
      situational.modelConfidence < 0.4
        ? 'low'
        : situational.modelConfidence < 0.6
          ? 'medium'
          : 'high';
  ...
  const overallRecommendation =
    confidenceData.roi > 0.05
      ? 'STRONG_BET' // 5%+ ROI is strong
      : confidenceData.roi > 0
        ? 'GOOD_BET' // Positive ROI is good
        : confidenceData.roi > -0.1
          ? 'WEAK_BET' // Slightly negative is weak
          : 'AVOID'; // Significantly negative is avoid

  // Calculate risk level
  const riskLevel = situational.oddsRange.includes('heavy')
    ? 'HIGH'
    : situational.oddsRange.includes('slight') || situational.oddsRange === 'toss_up'
      ? 'LOW'
      : 'MEDIUM';
  ...
  if (confidenceData.roi > 0) {
    keyInsights.push(
      `✅ ${confidenceData.description} historically profitable (+${(confidenceData.roi * 100).toFixed(1)}%)`,
    );
  } else {
    keyInsights.push(
      `❌ ${confidenceData.description} historically unprofitable (${(confidenceData.roi * 100).toFixed(1)}%)`,
    );
  }
...
export function formatHistoricalContext(context: HistoricalContext): string {
  const recommendation =
    context.overallRecommendation === 'STRONG_BET'
      ? '🟢 STRONG BET'
      : context.overallRecommendation === 'GOOD_BET'
        ? '🟡 GOOD BET'
        : context.overallRecommendation === 'WEAK_BET'
          ? '🟠 WEAK BET'
          : '🔴 AVOID';

  const risk =
    context.riskLevel === 'LOW' ? '🟢 LOW' : context.riskLevel === 'MEDIUM' ? '🟡 MED' : '🔴 HIGH';
