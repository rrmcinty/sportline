...
export function saveModel(
  ...
  if (model.modelType === 'logistic_regression') {
    ...
    if (logreg.theta && Array.isArray(logreg.theta)) {
      ...
      console.log(
        `[ModelStorage] Theta range: [${Math.min(...logreg.theta).toFixed(4)}, ${Math.max(...logreg.theta).toFixed(4)}]`,
      );
    }
...
export function loadModel(modelPath: string): TrainedModel {
  ...
  if (isVerbose) {
    ...
    console.log(
      `[ModelStorage] Test accuracy: ${(model.backtestMetrics.accuracy * 100).toFixed(2)}%`,
    );
 ...
 */
export function findLatestModel(sport: string, market: string, modelsDir: string): string | null {
  ...
  const modelFiles = files.filter(
    (f) => f.startsWith(`${sport}_${market}_`) && f.endsWith('.json'),
  );
 ...
 */
export function generateModelFilename(sport: string, market: string, date?: Date): string {
