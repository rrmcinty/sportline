...
function blendWithMarket(modelProb, marketProb) {
...
function getFetchEvents(sport) {
    if (sport === "cfb")
        return fetchEventsCfb;
    if (sport === "nfl")
        return fetchEventsNfl;
    if (sport === "nba")
        return fetchEventsNba;
    if (sport === "nhl")
        return fetchNHLEvents;
 ...
 */
export async function getHomeWinModelProbabilities(sport, date) {
    const db = getDb();
    const latestRun = db
        .prepare(`SELECT run_id, artifacts_path FROM model_runs WHERE sport = ? AND config_json LIKE '%moneyline%' ORDER BY finished_at DESC LIMIT 1`)
        .get(sport);
    if (!latestRun)
        return undefined;
    ...
    const rows = db
        .prepare(`SELECT id, espn_event_id FROM games WHERE sport = ? AND (date LIKE ? || '%' OR date LIKE ? || '%')`)
        .all(sport, isoPrefix, nextDayPrefix);
    if (rows.length === 0)
        return undefined;
    const probs = new Map();
    if (isEnsemble && baseModel && marketModel && ensembleConfig) {
        ...
        for (const r of rows) {
            ...
            const ensembleProb = ensembleConfig.baseWeight * baseProb +
        ...
        for (const r of rows) {
            ...
            const x = baseModel.featureNames.length === 9
                ? features.base
                : features.market;
 ...
 * Returns undefined if no model available or no games.
 */
export async function getHomeSpreadCoverProbabilities(sport, date) {
    const db = getDb();
    const latestRun = db
        .prepare(`SELECT run_id, artifacts_path FROM model_runs WHERE sport = ? AND config_json LIKE '%spread%' ORDER BY finished_at DESC LIMIT 1`)
        .get(sport);
    if (!latestRun)
        return undefined;
    const modelPath = join(latestRun.artifacts_path, "model.json");
    ...
    const nextDayPrefix = nextDay.toISOString().slice(0, 10);
    // Compute features for season
    const allFeatures = computeFeatures(db, sport, Array.isArray(model.seasons) && model.seasons.length
        ? model.seasons
        : [2024, 2025]);
    const featureMap = new Map();
    for (const f of allFeatures) {
 ...
 */
export async function getTotalOverModelProbabilities(sport, date) {
    const db = getDb();
    const latestRun = db
        .prepare(`SELECT run_id, artifacts_path FROM model_runs WHERE sport = ? AND config_json LIKE '%total%' ORDER BY finished_at DESC LIMIT 1`)
        .get(sport);
    if (!latestRun)
        return undefined;
    ...
    const rows = db
        .prepare(`SELECT id, espn_event_id FROM games WHERE sport = ? AND (date LIKE ? || '%' OR date LIKE ? || '%')`)
        .all(sport, isoPrefix, nextDayPrefixT);
    if (!rows.length)
        return undefined;
    const probs = new Map();
    if (useEnsemble) {
        ...
        if (model.predictionType === "regression" &&
            model.means &&
            model.stds &&
            model.sigma) {
