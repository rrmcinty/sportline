 ...
 */
export async function backtestMoneyline(
  sport: Sport,
  seasons: number[],
): Promise<void> {
  const db = getDb();
  ...
  const dateSet = new Set(
    games.map((g) => g.date.slice(0, 10).replace(/-/g, "")),
  );
  ...
  for (const game of games) {
    ...
    const homeDecimal =
      odds.price_home < 0
        ? 100 / Math.abs(odds.price_home) + 1
        : odds.price_home / 100 + 1;
    const awayDecimal =
      odds.price_away < 0
        ? 100 / Math.abs(odds.price_away) + 1
        : odds.price_away / 100 + 1;
  ...
  console.log(
    `Matched ${results.length} games with both predictions and odds\n`,
  );
  ...
  for (const bin of bins) {
    const binResults = results.filter(
      (r) => r.modelProb >= bin.min && r.modelProb < bin.max,
    );
    if (binResults.length === 0) continue;

    const avgPredicted =
      binResults.reduce((sum, r) => sum + r.modelProb, 0) / binResults.length;
  ...
  for (const c of calibration) {
    ...
    const roi =
      c.bets > 0 ? ((c.profit / (c.bets * 10)) * 100).toFixed(1) : "0.0";
    console.log(
      `${c.bin.padEnd(9)} | ${(c.predicted * 100).toFixed(1)}%`.padEnd(12) +
        `| ${(c.actual * 100).toFixed(1)}%`.padEnd(8) +
        `| ${c.count.toString().padEnd(5)} | ${(error * 100).toFixed(1)}%`.padEnd(
          8,
        ) +
  ...
  const totalError =
    calibration.reduce(
      (sum, c) => sum + Math.abs(c.predicted - c.actual) * c.count,
      0,
  ...
  console.log(
    `Total Profit (if bet all): ${totalProfit >= 0 ? "+" : ""}$${totalProfit.toFixed(2)} on ${totalBets} bets`,
  );
  console.log(
    `Return on Investment: ${totalROI >= 0 ? "+" : ""}${totalROI.toFixed(2)}%`,
  );
  console.log(
    "═══════════════════════════════════════════════════════════════════════\n",
  );
  ...
  for (const threshold of underdogThresholds) {
    ...
    const modelFavored = results.filter((r) => {
      const isHomeUnderdog = r.homeOdds >= threshold.minOdds;
      return (
        (isHomeUnderdog && r.modelProb > r.marketProb) ||
        (!isHomeUnderdog && r.modelProb < r.marketProb)
      );
    }).length;
    ...
    console.log(
      `  Correct picks: ${correctPicks}/${modelFavored} (${modelFavored > 0 ? ((correctPicks / modelFavored) * 100).toFixed(1) : "0.0"}%)`,
    );
    console.log(
      `  Profit if bet model's picks: ${profit >= 0 ? "+" : ""}$${profit.toFixed(2)}`,
    );
    console.log(
      `  ROI: ${modelFavored > 0 ? ((profit / (modelFavored * 10)) * 100).toFixed(1) : "0.0"}%`,
    );
  }
  ...
  console.log(
    `Analyzing ${winners.length} profitable bets vs ${losers.length} losing bets\n`,
  );

  // Key stats to analyze
  const statsToAnalyze = [
    {
      name: "Win Rate 5-game (Home)",
      getHome: (f: any) => f.homeWinRate5,
      getAway: (f: any) => f.awayWinRate5,
    },
    {
      name: "Win Rate 10-game (Home)",
      getHome: (f: any) => f.homeWinRate10,
      getAway: (f: any) => f.awayWinRate10,
    },
    {
      name: "Avg Margin 5-game (Home)",
      getHome: (f: any) => f.homeAvgMargin5,
      getAway: (f: any) => f.awayAvgMargin5,
    },
    {
      name: "Avg Margin 10-game (Home)",
      getHome: (f: any) => f.homeAvgMargin10,
      getAway: (f: any) => f.awayAvgMargin10,
    },
    {
      name: "Opp Win Rate 5-game (SoS)",
      getHome: (f: any) => f.homeOppWinRate5,
      getAway: (f: any) => f.awayOppWinRate5,
    },
    {
      name: "Opp Win Rate 10-game (SoS)",
      getHome: (f: any) => f.homeOppWinRate10,
      getAway: (f: any) => f.awayOppWinRate10,
    },
    {
      name: "Opp Margin 5-game (SoS Quality)",
      getHome: (f: any) => f.homeOppAvgMargin5,
      getAway: (f: any) => f.awayOppAvgMargin5,
    },
    {
      name: "Opp Margin 10-game (SoS Quality)",
      getHome: (f: any) => f.homeOppAvgMargin10,
      getAway: (f: any) => f.awayOppAvgMargin10,
    },
    {
      name: "Points Scored 5-game",
      getHome: (f: any) => f.homePointsAvg5,
      getAway: (f: any) => f.awayPointsAvg5,
    },
    {
      name: "Points Scored 10-game",
      getHome: (f: any) => f.homePointsAvg10,
      getAway: (f: any) => f.awayPointsAvg10,
    },
    {
      name: "Points Allowed 5-game (Defense)",
      getHome: (f: any) => f.homeDefEff5,
      getAway: (f: any) => f.awayDefEff5,
    },
    {
      name: "Points Allowed 10-game (Defense)",
      getHome: (f: any) => f.homeDefEff10,
      getAway: (f: any) => f.awayDefEff10,
    },
    {
      name: "Pace 5-game (Tempo)",
      getHome: (f: any) => f.homePace5,
      getAway: (f: any) => f.awayPace5,
    },
    {
      name: "Pace 10-game (Tempo)",
      getHome: (f: any) => f.homePace10,
      getAway: (f: any) => f.awayPace10,
    },
    {
      name: "Model-Market Divergence",
      getHome: (f: any, g: any) => g.divergence,
      getAway: (f: any, g: any) => g.divergence,
    },
  ];
  ...
  for (const stat of statsToAnalyze) {
    const winnerValues = winners.map((g) => {
      const val = g.betOnHome
        ? stat.getHome(g.features, g)
        : stat.getAway(g.features, g);
    ...
    const edge =
      diff > 0 ? "✓ Winners higher" : diff < 0 ? "✗ Losers higher" : "= Equal";
  ...
  const diffs = statsToAnalyze
    .map((stat) => {
      ...
      return {
        name: stat.name,
        diff: Math.abs(winnerAvg - loserAvg),
        direction: winnerAvg > loserAvg ? "higher" : "lower",
      };
    })
  ...
  for (let i = 0; i < Math.min(3, diffs.length); i++) {
    console.log(
      `   ${i + 1}. ${diffs[i].name} (winners ${diffs[i].direction} by ${diffs[i].diff.toFixed(3)})`,
    );
  }

  // Analyze high-confidence vs low-confidence bets
  const highConfidence = gamesWithFeatures.filter(
    (g) => Math.abs(g.modelProb - 0.5) > 0.2,
  );
  const lowConfidence = gamesWithFeatures.filter(
    (g) => Math.abs(g.modelProb - 0.5) <= 0.2,
  );

  const highConfWinRate =
    highConfidence.filter((g) => g.won).length / highConfidence.length;
  const lowConfWinRate =
    lowConfidence.filter((g) => g.won).length / lowConfidence.length;

  console.log(
    `\n   High-confidence bets (>70% or <30%): ${(highConfWinRate * 100).toFixed(1)}% win rate (${highConfidence.length} bets)`,
  );
  console.log(
    `   Low-confidence bets (40-60%): ${(lowConfWinRate * 100).toFixed(1)}% win rate (${lowConfidence.length} bets)`,
  );
 ...
 */
export async function backtestSpreads(
  sport: Sport,
  seasons: number[],
): Promise<void> {
  const db = getDb();
  ...
  for (const game of games) {
    ...
    if (!odds || odds.line === null || !odds.price_home || !odds.price_away)
      continue;
  ...
  const ece = calibration.reduce((sum, bin) => {
    return (
      sum + (bin.count / results.length) * Math.abs(bin.predicted - bin.actual)
    );
  }, 0);
  ...
  for (const threshold of thresholds) {
    const underdogBets = results.filter((r) => {
      ...
      const meetsThreshold =
        r.homeOdds >= threshold.minOdds || r.awayOdds >= threshold.minOdds;
    ...
    console.log(
      `${threshold.label}: ${wins}/${underdogBets.length} correct (${((wins / underdogBets.length) * 100).toFixed(1)}%), ROI: ${underdogRoi >= 0 ? "+" : ""}${underdogRoi.toFixed(1)}%`,
    );
  }
 ...
 */
export async function backtestTotals(
  sport: Sport,
  seasons: number[],
): Promise<void> {
  const db = getDb();
  ...
  for (const game of games) {
    ...
    const overDecimal =
      odds.price_over < 0
        ? 100 / Math.abs(odds.price_over) + 1
        : odds.price_over / 100 + 1;
    const underDecimal =
      odds.price_under < 0
        ? 100 / Math.abs(odds.price_under) + 1
        : odds.price_under / 100 + 1;
  ...
  const divergenceThresholds = [
    { label: "High divergence (>10%)", minDivergence: 0.1 },
    {
      label: "Moderate divergence (5-10%)",
      minDivergence: 0.05,
      maxDivergence: 0.1,
    },
  ...
  for (const threshold of divergenceThresholds) {
    const divergentGames = results.filter((r) => {
      ...
      if (threshold.maxDivergence !== undefined) {
        return (
          div < threshold.maxDivergence &&
          (threshold.minDivergence === undefined ||
            div >= threshold.minDivergence)
        );
      }
      return (
        threshold.minDivergence !== undefined && div >= threshold.minDivergence
      );
    });
    ...
    console.log(
      `  Correct picks: ${correctPicks}/${bets} (${bets > 0 ? ((correctPicks / bets) * 100).toFixed(1) : "0.0"}%)`,
    );
    console.log(
      `  Profit if bet disagreements: ${profit >= 0 ? "+" : ""}$${profit.toFixed(2)}`,
    );
    console.log(
      `  ROI: ${bets > 0 ? ((profit / (bets * 10)) * 100).toFixed(1) : "0.0"}%`,
    );
  }
  ...
  const statsToAnalyze = [
    {
      name: "Combined Points/Game 5-game",
      getValue: (f: any) => f.homePointsAvg5 + f.awayPointsAvg5,
    },
    {
      name: "Combined Points/Game 10-game",
      getValue: (f: any) => f.homePointsAvg10 + f.awayPointsAvg10,
    },
    {
      name: "Combined Pace 5-game",
      getValue: (f: any) => f.homePace5 + f.awayPace5,
    },
    {
      name: "Combined Pace 10-game",
      getValue: (f: any) => f.homePace10 + f.awayPace10,
    },
    {
      name: "Combined Defense 5-game",
      getValue: (f: any) => f.homeDefEff5 + f.awayDefEff5,
    },
    {
      name: "Combined Defense 10-game",
      getValue: (f: any) => f.homeDefEff10 + f.awayDefEff10,
    },
    { name: "Home Points/Game 5-game", getValue: (f: any) => f.homePointsAvg5 },
    { name: "Away Points/Game 5-game", getValue: (f: any) => f.awayPointsAvg5 },
    {
      name: "Home Points/Game 10-game",
      getValue: (f: any) => f.homePointsAvg10,
    },
    {
      name: "Away Points/Game 10-game",
      getValue: (f: any) => f.awayPointsAvg10,
    },
    ...
    {
      name: "Model-Market Divergence",
      getValue: (f: any, g: any) => g.divergence,
    },
  ];
