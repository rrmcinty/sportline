...
interface ModelMeta { run_id:string; artifacts_path:string; metrics_json:string; config_json:string; }
...
function binCalibration(preds:number[], labels:number[], bins:number=10) {
  const result: { bin:string; count:number; avgPred:number; avgActual:number }[] = [];
  for (let b=0; b<bins; b++) {
    const lo = b / bins;
    const hi = (b+1)/bins + 1e-9;
    const idxs = preds.map((p,i)=>({p,i})).filter(o=> o.p >= lo && o.p < hi);
    if (!idxs.length) continue;
    const avgPred = idxs.reduce((a,o)=>a+o.p,0)/idxs.length;
    const avgActual = idxs.reduce((a,o)=>a+labels[o.i],0)/idxs.length;
    result.push({ bin:`${(lo*100).toFixed(0)}-${(hi*100).toFixed(0)}%`, count: idxs.length, avgPred, avgActual });
  }
  const ece = result.reduce((acc,r)=> acc + (r.count/preds.length)*Math.abs(r.avgPred - r.avgActual), 0);
  return { bins: result, ece };
}

function summaryStats(values:number[]) {
  const n = values.length; if(!n) return { n:0, mean:0, std:0, min:0, max:0, p10:0,p50:0,p90:0};
  const mean = values.reduce((a,b)=>a+b,0)/n;
  const variance = values.reduce((a,b)=>a + (b-mean)*(b-mean),0)/n;
  const std = Math.sqrt(variance);
  return { n, mean, std, min: Math.min(...values), max: Math.max(...values), p10:percentile(values,10), p50:percentile(values,50), p90:percentile(values,90)};
}

function printCalibration(title:string, cal:{bins:{bin:string;count:number;avgPred:number;avgActual:number}[]; ece:number}) {
  console.log(chalk.bold(title));
  for (const b of cal.bins) {
    console.log(chalk.dim(`  Bin ${b.bin} n=${b.count} pred=${(b.avgPred*100).toFixed(1)}% actual=${(b.avgActual*100).toFixed(1)}%`));
  }
...
async function runDiagnostics() {
  ...
  if (moneylineModel) {
    ...
    console.log(chalk.magenta(` Divergence (model - market) mean=${(divStats.mean*100).toFixed(2)}% std=${(divStats.std*100).toFixed(2)}% min=${(divStats.min*100).toFixed(2)}% max=${(divStats.max*100).toFixed(2)}% p90=${(divStats.p90*100).toFixed(2)}%`));
    console.log();
  }
  ...
  if (spreadModel) {
    ...
    if (m.market === 'spread') {
      ...
      const valGames = features.filter(f=> f.date >= valDate && gameMap.has(f.gameId) && f.spreadLine !== null && f.spreadMarketImpliedProb !== null);
      const preds:number[] = []; const labels:number[] = []; const divergences:number[] = [];
      for (const f of valGames) {
        const x = [f.homeWinRate5,f.awayWinRate5,f.homeAvgMargin5,f.awayAvgMargin5,f.homeAdvantage,f.homeOppWinRate5,f.awayOppWinRate5,f.homeOppAvgMargin5,f.awayOppAvgMargin5,f.spreadLine!,f.spreadMarketImpliedProb!];
        const z = x.reduce((acc,v,i)=> acc + v * m.weights[i],0);
        const p = 1/(1+Math.exp(-z));
        const game = gameMap.get(f.gameId)!;
      ...
      console.log(chalk.dim(` Validation games: ${preds.length}`));
      const cal = binCalibration(preds, labels, 10); printCalibration(' Spread Calibration (10 bins)', cal);
      const divStats = summaryStats(divergences);
      console.log(chalk.magenta(` Divergence (model - market spread prob) mean=${(divStats.mean*100).toFixed(2)}% std=${(divStats.std*100).toFixed(2)}% p90=${(divStats.p90*100).toFixed(2)}%`));
      console.log();
    }
  }
  ...
  if (totalModel) {
    ...
    if (m.predictionType === 'regression') {
      ...
      const valGames = features.filter(f=> f.date >= valDate && gameMap.has(f.gameId) && f.totalLine !== null);
      const predsOver:number[] = []; const labelsOver:number[] = []; const residuals:number[] = []; const divergences:number[] = [];
      for (const f of valGames) {
        ...
        const scaled = featureVector.map((v:number,i:number)=> (v - m.means[i]) / m.stds[i]);
        const bias = (m as any).bias || 0;
        const predictedScore = scaled.reduce((acc:number,v:number,i:number)=> acc + v * m.weights[i],0) + bias;
        const game = gameMap.get(f.gameId)!;
        ...
        predsOver.push(pOver); labelsOver.push(labelOver);
        residuals.push(actualCombined - predictedScore);
        if (f.totalMarketImpliedProb !== null) divergences.push(pOver - f.totalMarketImpliedProb);
      }
      console.log(chalk.bold.green('Totals Model (Regression)'));
      console.log(chalk.dim(` Validation games: ${predsOver.length}`));
      const cal = binCalibration(predsOver, labelsOver, 10); printCalibration(' Totals Over Probability Calibration (10 bins)', cal);
      const resStats = summaryStats(residuals);
      console.log(chalk.yellow(` Residuals mean=${resStats.mean.toFixed(2)} std=${resStats.std.toFixed(2)} p10=${resStats.p10.toFixed(2)} p90=${resStats.p90.toFixed(2)} range=[${resStats.min.toFixed(2)}, ${resStats.max.toFixed(2)}]`));
      if (divergences.length) {
        const divStats = summaryStats(divergences);
        console.log(chalk.magenta(` Divergence (model - market total over prob) mean=${(divStats.mean*100).toFixed(2)}% std=${(divStats.std*100).toFixed(2)}% p90=${(divStats.p90*100).toFixed(2)}%`));
      }
      console.log();
    }
  }
...
runDiagnostics().catch(err => { console.error('Diagnostics failed', err); process.exit(1); });
