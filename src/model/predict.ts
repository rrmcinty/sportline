...
export async function cmdModelPredict(
  ...
  try {
    ...
    if (!latestRun) {
      console.log(
        chalk.yellow("⚠️  No trained model found. Run 'model train' first."),
      );
      return;
    }
    ...
    if (metrics) {
      ...
      if (
        metrics.numTrainingSamples !== undefined &&
        metrics.numValidationSamples !== undefined
      ) {
        console.log(
          chalk.magenta(`  Training samples: ${metrics.numTrainingSamples}`),
        );
        console.log(
          chalk.magenta(
            `  Validation samples: ${metrics.numValidationSamples}`,
          ),
        );
      }
      if (metrics.validationAccuracy !== undefined) {
        console.log(
          chalk.magenta(
            `  Validation accuracy: ${metrics.validationAccuracy.toFixed(2)}%`,
          ),
        );
      }
      if (metrics.brierScore !== undefined) {
        console.log(
          chalk.magenta(`  Brier score: ${metrics.brierScore.toFixed(4)}`),
        );
      }
      ...
      if (metrics.splitDate) {
        console.log(
          chalk.magenta(`  Train/validation split date: ${metrics.splitDate}`),
        );
      }
      console.log();
    }
    ...
    for (const comp of competitions) {
      const home = upsertTeam.get(
        sport,
        comp.homeTeam.id,
        comp.homeTeam.name,
        comp.homeTeam.abbreviation || null,
      ) as { id: number };
      const away = upsertTeam.get(
        sport,
        comp.awayTeam.id,
        comp.awayTeam.name,
    ...
    console.log(
      chalk.gray(
        `Using model trained on season${model.seasons && model.seasons.length > 1 ? "s" : ""} ${model.seasons ? model.seasons.join(", ") : model.season} (${model.trainedAt})`,
      ),
    );
    ...
    const scored = rows
      .map((r) => {
        const x = featureMap.get(r.game_id) || [
          0.5, 0.5, 0, 0, 1, 0.5, 0.5, 0, 0, 0.5, 0.5, 0, 0, 0.5, 0.5, 0, 0,
          0.5,
        ];
    ...
    for (const s of scored) {
      ...
      const home =
        s.pHome >= 0.5
          ? chalk.green.bold(s.home_name)
          : chalk.bold(s.home_name);
      ...
      const probColor =
        s.pHome >= 0.5 ? chalk.green(winnerProb) : chalk.yellow(winnerProb);
      // Show prediction line without event ID (odds are now shown below)
      console.log(
        `${away} @ ${home}  →  Home win: ${probColor}  (${new Date(s.date).toLocaleString()})`,
      );
      ...
      if (fetchOdds && normalizeOdds) {
        try {
          ...
          if (oddsEntries && oddsEntries.length > 0) {
            ...
            console.log(
              chalk.gray("  No market odds available for this event."),
            );
          }
        } catch (err) {
          let msg = "";
    ...
    console.log(
      chalk.dim(
        "💡 Tip: Use 'odds --sport <sport> --event <id> --date <yyyymmdd>' to compare with market.",
      ),
    );
  } catch (error) {
