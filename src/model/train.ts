...
async function trainMoneylineModel(
  ...
  console.log(
    chalk.blue(`[DEBUG] Received ${gameFeatures.length} gameFeatures`),
  );
  ...
  for (const gf of gameFeatures) {
    ...
    if (fgPctDebugCount < 10 && typeof gf.fg_pct === "number") {
      console.log(
        `[DEBUG] gameId=${gf.gameId}, date=${gf.date}, fg_pct=${gf.fg_pct}`,
      );
      fgPctDebugCount++;
    }
  ...
  console.log(
    chalk.blue(
      `[DEBUG] Skipped ${skippedNoOutcome} games due to missing outcome, ${skippedNaN} due to NaN/undefined features`,
    ),
  );
