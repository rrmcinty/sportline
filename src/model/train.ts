// Utility to print basketball feature summary stats
function printBasketballFeatureStats(gameFeatures: any[]) {
  const statKeys = [
    "home_fg_pct",
    "away_fg_pct",
    "home_fg3_pct",
  ...
  for (const key of statKeys) statSummary[key] = [];
  for (const gf of gameFeatures) {
    for (const key of statKeys) {
      if (typeof gf[key] === "number" && !Number.isNaN(gf[key])) {
        statSummary[key].push(gf[key]);
      }
    }
  }
  console.log("[SUMMARY] Basketball feature stats:");
  for (const key of statKeys) {
    const arr = statSummary[key];
    if (arr.length === 0) {
    ...
    console.log(
      `  ${key}: count=${arr.length}, min=${min.toFixed(2)}, max=${max.toFixed(2)}, mean=${mean.toFixed(2)}, std=${std.toFixed(2)}`,
    );
  }
}
...
function trainLogisticRegression(
  ...
  const weightSum = useWeights
    ? (sampleWeights as number[]).reduce((a, b) => a + b, 0)
    : n;
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    for (let j = 0; j < numFeatures; j++) {
      weights[j] -=
        (learningRate / weightSum) * (gradient[j] + lambda * weights[j]);
    }
  }
  ...
  try {
    ...
    for (const market of markets) {
      ...
      if (shouldAvoidMarket(sport, market)) {
        ...
        console.log(
          chalk.yellow(
            `   Training anyway, but this market is NOT recommended for production use.\n`,
          ),
        );
      }
      ...
      if (optimalConfig && (!seasons || seasons.length === 0)) {
        console.log(
          chalk.dim(`   Using optimal config: ${optimalConfig.reason}`),
        );
        console.log(
          chalk.dim(
            `   Expected: ${optimalConfig.expectedROI >= 0 ? "+" : ""}${optimalConfig.expectedROI.toFixed(2)}% ROI, ${optimalConfig.expectedECE.toFixed(2)}% ECE`,
          ),
        );
      }
      ...
      console.log(
        chalk.dim(`Features computed for ${gameFeatures.length} games\n`),
      );

      if (market === "moneyline") {
        await trainMoneylineModel(
          db,
          sport,
          useSeasons,
          gameFeatures,
        ...
        await trainTotalClassificationModel(
          db,
          sport,
          useSeasons,
          gameFeatures,
...
async function trainMoneylineModel(
  ...
  console.log(
    chalk.blue(
      `[DEBUG] Loaded ${gamesWithOutcomes.length} completed games with outcomes for seasons: ${seasons.join(", ")}`,
    ),
  );
  // if (gamesWithOutcomes.length > 0) {
  //   console.log(
  //     chalk.blue(`[DEBUG] Sample gameWithOutcome: `),
  //     gamesWithOutcomes[0],
  //   );
  // }
  ...
  if (gamesWithOutcomes.length === 0) {
    console.log(
      chalk.yellow(
        "⚠️  No completed games found. Run data ingest with completed games first.",
      ),
    );
    return;
  }
  ...
  for (const gf of gameFeatures) {
    ...
    const baseFeat = [
      ...
      // Basketball-specific features (now using home/away keys)
      gf.home_fg_pct,
      gf.away_fg_pct,
      gf.home_fg3_pct,
      gf.away_fg3_pct,
      gf.home_turnovers,
      gf.away_turnovers,
      gf.home_fgs_attempted,
      gf.away_fgs_attempted,
      gf.home_steals,
      gf.away_steals,
    ];
  ...
  console.log(
    chalk.dim(`Training on ${trainingData.features.length} completed games\n`),
  );

  if (trainingData.features.length < 10) {
    // Print basketball feature summary stats after feature extraction
    printBasketballFeatureStats(gameFeatures);
    console.log(
      chalk.yellow(
        "⚠️  Not enough completed games (<10). Model training skipped.",
      ),
    );
    return;
  }
  ...
  console.log(
    chalk.dim(
      `Temporal split at ${splitDate}: ${trainIndices.length} train, ${valIndices.length} validation\n`,
    ),
  );
  ...
  console.log(
    chalk.dim("Training MARKET-AWARE model (with market feature)..."),
  );
  ...
  const valPredictionsMarket = marketVal.map((f) =>
    sigmoid(dot(f, marketWeights)),
  );
  const valPredictionsEnsemble = valPredictionsBase.map(
    (pb, i) =>
      ensembleWeight.base * pb +
      ensembleWeight.market * valPredictionsMarket[i],
  );
  ...
  function computeMetrics(
    ...
    console.log(
      chalk.cyan(
        `  ${name}: Acc ${accuracy.toFixed(1)}%, Brier ${brierScore.toFixed(4)}, LogLoss ${logLoss.toFixed(4)}`,
      ),
    );
  ...
  const marketMetrics = computeMetrics(
    valPredictionsMarket,
    marketValLabels,
    "Market-Aware",
  );
  const ensembleMetrics = computeMetrics(
    valPredictionsEnsemble,
    baseValLabels,
    "Ensemble (70/30)",
  );
  ...
  for (let b = 0; b < bins.length - 1; b++) {
    ...
    for (let i = 0; i < valPredictionsEnsemble.length; i++) {
      if (
        valPredictionsEnsemble[i] >= binStart &&
        valPredictionsEnsemble[i] < binEnd
      ) {
        binSamples.push({
          pred: valPredictionsEnsemble[i],
          actual: baseValLabels[i],
        });
      }
    }
    ...
    if (binSamples.length > 0) {
      const avgPred =
        binSamples.reduce((sum, s) => sum + s.pred, 0) / binSamples.length;
  ...
  console.log(
    chalk.cyan(
      `Brier score: ${brierScore.toFixed(4)} (lower is better, 0.25 = random)`,
    ),
  );
  console.log(
    chalk.cyan(`Log loss: ${logLoss.toFixed(4)} (lower is better)\n`),
  );
  ...
  writeFileSync(
    join(artifactsPath, "base_model.json"),
  ...
  writeFileSync(
    join(artifactsPath, "market_model.json"),
    JSON.stringify(marketModel, null, 2),
  );
  ...
  writeFileSync(
    join(artifactsPath, "ensemble.json"),
    JSON.stringify(ensembleConfig, null, 2),
  );
  ...
  const marketTrainFeatures = trainIndices.map(
    (i) => marketAwareData.features[i],
  );
  ...
  for (let j = 0; j < means.length; j++) {
    for (let i = 0; i < marketTrainFeatures.length; i++)
      means[j] += marketTrainFeatures[i][j];
  ...
  writeFileSync(
    join(artifactsPath, "model.json"),
    JSON.stringify(unifiedModel, null, 2),
  );
  ...
  writeFileSync(
    join(artifactsPath, "metrics.json"),
  ...
  console.log(
    chalk.green.bold(
      `✅ Moneyline ensemble trained and saved to ${artifactsPath}\n`,
    ),
  );
}
...
async function trainSpreadModel(
  ...
  if (gamesWithSpreads.length === 0) {
    console.log(
      chalk.yellow("⚠️  No completed games with spread lines found.\n"),
    );
    return;
  }
  ...
  for (const gf of gameFeatures) {
    ...
    if (
      outcome !== undefined &&
      gf.spreadLine !== null &&
      gf.spreadMarketImpliedProb !== null
    ) {
  ...
  console.log(
    chalk.dim(
      `Training on ${trainingData.features.length} completed games with spreads\n`,
    ),
  );
  ...
  if (trainingData.features.length < 10) {
    console.log(
      chalk.yellow(
        "⚠️  Not enough completed games with spreads (<10). Model training skipped.\n",
      ),
    );
    return;
  }
  ...
  console.log(
    chalk.dim(
      `Temporal split at ${splitDate}: ${trainFeatures.length} train, ${valFeatures.length} validation\n`,
    ),
  );
  ...
  if (calibrate === "isotonic" && valFeatures.length >= 400) {
    ...
    console.log(
      chalk.green(
        `Calibration fitted with ${calibrationCurve.x.length} points\n`,
      ),
    );
  } else {
    console.log(
      chalk.yellow(
        `⚠️  Skipping calibration (need ≥400 validation samples for stable isotonic regression, have ${valFeatures.length})\n`,
      ),
    );
  }
  ...
  console.log(
    chalk.green.bold(`✅ Spread model trained and saved to ${artifactsPath}\n`),
  );
}
...
async function trainTotalClassificationModel(
  ...
  const combinedScoreMap = new Map(
    gamesWithTotals.map((g) => [g.id, g.home_score + g.away_score]),
  );
  ...
  console.log(
    chalk.dim(
      `  Split: ${baseXtrain.length} train, ${baseXval.length} validation`,
    ),
  );
  ...
  const scaleBase = (row: number[]) =>
    row.map((v, j) => (v - baseMeans[j]) / baseStds[j]);
  ...
  console.log(
    chalk.green(`  Base model validation accuracy: ${baseValAcc.toFixed(1)}%`),
  );
  console.log(
    chalk.cyan(`  Base model Brier score: ${baseValBrier.toFixed(4)}\n`),
  );

  // ========== TRAIN MARKET-AWARE MODEL ==========
  console.log(
    chalk.bold("Training market-aware model (with totalMarketImpliedProb)..."),
  );
  ...
  console.log(
    chalk.dim(
      `  Split: ${marketXtrain.length} train, ${marketXval.length} validation`,
    ),
  );
  ...
  for (let j = 0; j < marketMeans.length; j++) {
    for (let i = 0; i < marketXtrain.length; i++)
      marketMeans[j] += marketXtrain[i][j];
  ...
  const scaleMarket = (row: number[]) =>
    row.map((v, j) => (v - marketMeans[j]) / marketStds[j]);
  ...
  // Validation metrics
  const marketValPreds = marketXvalScaled.map((x) =>
    sigmoid(dot(x, marketWeights)),
  );
  let marketCorrect = 0;
  ...
  console.log(
    chalk.green(
      `  Market-aware validation accuracy: ${marketValAcc.toFixed(1)}%`,
    ),
  );
  console.log(
    chalk.cyan(`  Market-aware Brier score: ${marketValBrier.toFixed(4)}\n`),
  );

  // ========== ENSEMBLE (70% base + 30% market-aware) ==========
  console.log(
    chalk.bold("Computing ensemble predictions (70% base + 30% market)..."),
  );
  ...
  const ensembleMarketValPreds = marketXvalScaled.map((x) =>
    sigmoid(dot(x, marketWeights)),
  );

  const ensembleValPreds = ensembleBaseValPreds.map(
    (base, i) => 0.7 * base + 0.3 * ensembleMarketValPreds[i],
  );
  ...
  console.log(
    chalk.green.bold(
      `  ✓ Ensemble validation accuracy: ${ensembleValAcc.toFixed(1)}%`,
    ),
  );
  console.log(
    chalk.cyan.bold(
      `  ✓ Ensemble Brier score: ${ensembleValBrier.toFixed(4)}\n`,
    ),
  );
  ...
  console.log(
    chalk.green.bold(
      `✅ Total classification ensemble trained and saved to ${artifactsPath}\n`,
    ),
  );
}
