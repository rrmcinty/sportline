...
function setHalfLife(halfLife: number) {
  ...
  const updated = content
    .replace(
      /computeRecencyWeights\(baseTrainDates, 120\)/g,
      `computeRecencyWeights(baseTrainDates, ${halfLife})`,
    )
    .replace(
      /computeRecencyWeights\(marketTrainDates, 120\)/g,
      `computeRecencyWeights(marketTrainDates, ${halfLife})`,
    )
    .replace(
      /computeRecencyWeights\(trainDates, 120\)/g,
      `computeRecencyWeights(trainDates, ${halfLife})`,
    );
...
function trainAndBacktest(
  sport: string,
  halfLife: number,
): BacktestResult | null {
  try {
    console.log(
      `📊 Training ${sport.toUpperCase()} with half-life ${halfLife}d...`,
    );
    execSync(
      `sportline model train --sport ${sport} --season 2024,2025 --markets moneyline`,
      {
    ...
    const output = execSync(
      `sportline model backtest --sport ${sport} --season 2024,2025 --market moneyline`,
      {
        cwd: resolve(__dirname, "../.."),
        encoding: "utf8",
      },
...
function formatResults(results: BacktestResult[]) {
  ...
  for (const sport of SPORTS) {
    ...
    if (baseline) {
      ...
      console.log(
        `${sport.toUpperCase()}: Use ${best.halfLife}d (${deltaStr} vs 120d baseline)`,
      );
    } else {
      console.log(
        `${sport.toUpperCase()}: Use ${best.halfLife}d (best ROI: ${best.roi.toFixed(2)}%)`,
      );
    }
  }
