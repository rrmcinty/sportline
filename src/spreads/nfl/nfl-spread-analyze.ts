...
function loadLatestModel(
  seasons: number[],
): { baseWeights: number[]; marketWeights: number[] } | null {
...
export async function analyzeNFLSpreadTraits(
  ...
  if (bucketsToAnalyze.length === 0) {
    console.log(
      chalk.yellow(
        "⚠️  No profitable buckets found. Run backtest first to identify profitable ranges.\n",
      ),
    );
    return;
  }
  ...
  console.log(
    chalk.bold(
      `Analyzing ${bucketsToAnalyze.length} bucket(s): ${bucketsToAnalyze.join(", ")}\n`,
    ),
  );
  ...
  for (const bucket of bucketsToAnalyze) {
    ...
    console.log(
      chalk.dim(`   Winners: ${winners.length} | Losers: ${losers.length}\n`),
    );

    // Compute average features for winners
    const winnerFeatures = {
      homeATSRecord5:
        winners.reduce((sum, r) => sum + r.game.homeATSRecord5, 0) /
        winners.length,
      awayATSRecord5:
        winners.reduce((sum, r) => sum + r.game.awayATSRecord5, 0) /
        winners.length,
      homeATSMargin5:
        winners.reduce((sum, r) => sum + r.game.homeATSMargin5, 0) /
        winners.length,
      awayATSMargin5:
        winners.reduce((sum, r) => sum + r.game.awayATSMargin5, 0) /
        winners.length,
      spreadSize:
        winners.reduce((sum, r) => sum + r.game.spreadSize, 0) / winners.length,
      isTightSpread:
        winners.filter((r) => r.game.isTightSpread === 1).length /
        winners.length,
      marketOverreaction:
        winners.reduce((sum, r) => sum + r.game.marketOverreaction, 0) /
        winners.length,
      homeMarginDelta:
        winners.reduce(
          (sum, r) => sum + (r.game.homeAvgMargin10 - r.game.awayAvgMargin10),
          0,
        ) / winners.length,
    };
    ...
    const loserFeatures = {
      homeATSRecord5:
        losers.reduce((sum, r) => sum + r.game.homeATSRecord5, 0) /
        losers.length,
      awayATSRecord5:
        losers.reduce((sum, r) => sum + r.game.awayATSRecord5, 0) /
        losers.length,
      homeATSMargin5:
        losers.reduce((sum, r) => sum + r.game.homeATSMargin5, 0) /
        losers.length,
      awayATSMargin5:
        losers.reduce((sum, r) => sum + r.game.awayATSMargin5, 0) /
        losers.length,
      spreadSize:
        losers.reduce((sum, r) => sum + r.game.spreadSize, 0) / losers.length,
      isTightSpread:
        losers.filter((r) => r.game.isTightSpread === 1).length / losers.length,
      marketOverreaction:
        losers.reduce((sum, r) => sum + r.game.marketOverreaction, 0) /
        losers.length,
      homeMarginDelta:
        losers.reduce(
          (sum, r) => sum + (r.game.homeAvgMargin10 - r.game.awayAvgMargin10),
    ...
    console.log(
      `   Home ATS Record (5g): ${(winnerFeatures.homeATSRecord5 * 100).toFixed(1)}%`,
    );
    console.log(
      `   Away ATS Record (5g): ${(winnerFeatures.awayATSRecord5 * 100).toFixed(1)}%`,
    );
    console.log(
      `   Home ATS Margin (5g): ${winnerFeatures.homeATSMargin5 >= 0 ? "+" : ""}${winnerFeatures.homeATSMargin5.toFixed(2)}`,
    );
    console.log(
      `   Away ATS Margin (5g): ${winnerFeatures.awayATSMargin5 >= 0 ? "+" : ""}${winnerFeatures.awayATSMargin5.toFixed(2)}`,
    );
    console.log(`   Spread Size: ${winnerFeatures.spreadSize.toFixed(1)}`);
    console.log(
      `   Tight Spread (≤3): ${(winnerFeatures.isTightSpread * 100).toFixed(1)}%`,
    );
    console.log(
      `   Market Overreaction: ${winnerFeatures.marketOverreaction.toFixed(2)}`,
    );
    console.log(
      `   Home Margin Delta: ${winnerFeatures.homeMarginDelta >= 0 ? "+" : ""}${winnerFeatures.homeMarginDelta.toFixed(2)}\n`,
    );

    console.log(chalk.bold.red("❌ Losing Bets - Average Features:"));
    console.log(
      `   Home ATS Record (5g): ${(loserFeatures.homeATSRecord5 * 100).toFixed(1)}%`,
    );
    console.log(
      `   Away ATS Record (5g): ${(loserFeatures.awayATSRecord5 * 100).toFixed(1)}%`,
    );
    console.log(
      `   Home ATS Margin (5g): ${loserFeatures.homeATSMargin5 >= 0 ? "+" : ""}${loserFeatures.homeATSMargin5.toFixed(2)}`,
    );
    console.log(
      `   Away ATS Margin (5g): ${loserFeatures.awayATSMargin5 >= 0 ? "+" : ""}${loserFeatures.awayATSMargin5.toFixed(2)}`,
    );
    console.log(`   Spread Size: ${loserFeatures.spreadSize.toFixed(1)}`);
    console.log(
      `   Tight Spread (≤3): ${(loserFeatures.isTightSpread * 100).toFixed(1)}%`,
    );
    console.log(
      `   Market Overreaction: ${loserFeatures.marketOverreaction.toFixed(2)}`,
    );
    console.log(
      `   Home Margin Delta: ${loserFeatures.homeMarginDelta >= 0 ? "+" : ""}${loserFeatures.homeMarginDelta.toFixed(2)}\n`,
    );

    // Highlight key differences
    console.log(chalk.bold.yellow("🔍 Key Differentiators:"));

    const atsDiff = Math.abs(
      winnerFeatures.homeATSRecord5 - loserFeatures.homeATSRecord5,
    );
    if (atsDiff > 0.05) {
      console.log(
        `   • ATS Record: Winners have ${winnerFeatures.homeATSRecord5 > loserFeatures.homeATSRecord5 ? "stronger" : "weaker"} home ATS trend (${(atsDiff * 100).toFixed(1)}% difference)`,
      );
    }

    const spreadDiff = Math.abs(
      winnerFeatures.spreadSize - loserFeatures.spreadSize,
    );
    if (spreadDiff > 1.0) {
      console.log(
        `   • Spread Size: Winners avg ${winnerFeatures.spreadSize.toFixed(1)}, losers avg ${loserFeatures.spreadSize.toFixed(1)}`,
      );
    }

    const tightSpreadDiff = Math.abs(
      winnerFeatures.isTightSpread - loserFeatures.isTightSpread,
    );
    if (tightSpreadDiff > 0.1) {
      console.log(
        `   • Tight Spreads (≤3): ${winnerFeatures.isTightSpread > loserFeatures.isTightSpread ? "More common" : "Less common"} in winners (${(tightSpreadDiff * 100).toFixed(1)}% difference)`,
      );
    }

    const marginDeltaDiff = Math.abs(
      winnerFeatures.homeMarginDelta - loserFeatures.homeMarginDelta,
    );
    if (marginDeltaDiff > 2.0) {
      console.log(
        `   • Margin Delta: Winners have ${winnerFeatures.homeMarginDelta > loserFeatures.homeMarginDelta ? "stronger" : "weaker"} home team relative performance`,
      );
    }
  }
