...
import type { NFLSpreadBacktestResults, NFLSpreadGameFeatures, SpreadConfidenceBucket } from "./types.js";
 ...
 */
function loadLatestModel(seasons: number[]): { baseWeights: number[]; marketWeights: number[]; modelDir: string } | null {
...
export async function backtestNFLSpreadModel(seasons: number[]): Promise<void> {
  ...
  console.log(`Analyzing ${completedGames.length} completed games with spreads\n`);
  ...
  const brier = predictions.reduce((sum, p, i) => sum + Math.pow(p - actuals[i], 2), 0) / predictions.length;
  const logLoss = -predictions.reduce((sum, p, i) => {
    const clipped = Math.max(1e-15, Math.min(1 - 1e-15, p));
    return sum + actuals[i] * Math.log(clipped) + (1 - actuals[i]) * Math.log(1 - clipped);
  ...
  for (const bucket of buckets) {
    ...
    const bucketPushes = results.filter(r => r.bucket === bucket && r.outcome === -1).length;
    ...
    console.log(`${roiIndicator} ${bucket.padEnd(10)} | ${bucketGames.length.toString().padStart(3)} bets | ${(bucketWinRate * 100).toFixed(1)}% win rate | ${roiColor(`${bucketROI >= 0 ? '+' : ''}${bucketROI.toFixed(1)}% ROI`)}`);
  }
  ...
  for (const range of spreadRanges) {
    const rangeGames = resultsNoPush.filter(r => r.spreadSize >= range.min && r.spreadSize <= range.max);
    ...
    console.log(`${roiIndicator} ${range.label.padEnd(8)} | ${rangeGames.length.toString().padStart(3)} bets | ${(rangeWinRate * 100).toFixed(1)}% win rate | ${roiColor(`${rangeROI >= 0 ? '+' : ''}${rangeROI.toFixed(1)}% ROI`)}`);
  }
  ...
  if (profitableBuckets.length > 0) {
    ...
    for (const bucket of profitableBuckets) {
      console.log(chalk.green(`   ${bucket.bucket}: +${bucket.roi.toFixed(1)}% ROI (${bucket.bets} bets, ${(bucket.winRate * 100).toFixed(1)}% win rate)`));
    }
    console.log();
  } else {
