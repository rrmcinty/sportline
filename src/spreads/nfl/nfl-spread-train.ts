...
function isotonicCalibration(
  ...
  for (let i = 0; i < pairs.length; i += binSize) {
    ...
    calibrationMap.push({
      predRange: [minPred, maxPred],
      calibratedProb: avgActual,
    });
  }
...
export async function trainNFLSpreadModel(
  ...
  console.log(
    chalk.dim(
      `   Approach: Base + market-aware ensemble with isotonic calibration\n`,
    ),
  );
  ...
  if (completedGames.length < 100) {
    console.log(
      chalk.yellow(
        `⚠️  Insufficient data (${completedGames.length} games). Need at least 100 completed games.`,
      ),
    );
    return;
  }
  ...
  const trainLabels = trainGames
    .map(getLabel)
    .filter((l): l is number => l !== null);
  const trainFeats = trainGames.filter(
    (_, i) => getLabel(trainGames[i]) !== null,
  );

  const valLabels = valGames
    .map(getLabel)
    .filter((l): l is number => l !== null);
  ...
  const ensemblePreds = basePreds.map(
    (bp, i) => 0.5 * bp + 0.5 * marketPreds[i],
  );
  ...
  if (valLabels.length >= 400) {
    ...
    console.log(
      chalk.yellow(
        `⚠️  Skipping calibration (need ≥400 validation samples, have ${valLabels.length})\n`,
      ),
    );
  }
  ...
  // Compute metrics
  const accuracy =
    calibratedPreds.filter((p, i) => (p >= 0.5 ? 1 : 0) === valLabels[i])
      .length / valLabels.length;
  const brier =
    calibratedPreds.reduce(
      (sum, p, i) => sum + Math.pow(p - valLabels[i], 2),
      0,
    ) / valLabels.length;
  const logLoss =
    -calibratedPreds.reduce((sum, p, i) => {
      const clipped = Math.max(1e-15, Math.min(1 - 1e-15, p));
      return (
        sum +
        valLabels[i] * Math.log(clipped) +
        (1 - valLabels[i]) * Math.log(1 - clipped)
      );
    }, 0) / valLabels.length;
  ...
  const modelPath = join(
    modelDir,
    `nfl_spread_${seasons.join("-")}_${timestamp}`,
  );
  mkdirSync(modelPath, { recursive: true });

  writeFileSync(
    join(modelPath, "base_model.json"),
    JSON.stringify(baseModel, null, 2),
  );
  writeFileSync(
    join(modelPath, "market_model.json"),
