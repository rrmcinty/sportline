 ...
 */
export async function loadTrackedBets() {
    try {
 ...
 */
export async function saveTrackedBets(data) {
    try {
        await fs.mkdir(path.dirname(TRACKING_FILE), { recursive: true });
 ...
 */
export async function logRecommendedBet(bet) {
 ...
 */
export async function markBetsAsPlaced(betIds, stakes) {
    const data = await loadTrackedBets();
 ...
 */
export async function updateBetResults(betId, homeScore, awayScore) {
    const data = await loadTrackedBets();
    const bet = data.bets.find((b) => b.id === betId);
    ...
    // Calculate profit
    let actualProfit = 0;
    const totalStake = (bet.placements || []).reduce((sum, p) => sum + p.stake, 0) ||
        bet.stake ||
        0;
...
export async function getBetStats() {
    ...
    const totalStaked = actualBets.reduce((sum, b) => sum +
        ((b.placements || []).reduce((s, p) => s + p.stake, 0) || b.stake || 0), 0);
...
//# sourceMappingURL=bet-logger.js.map