 ...
 */
export function analyzeWinningUnderdogs(sport, seasons, tiers, minOdds, maxOdds) {
    ...
    for (const game of winners) {
        ...
        const favoriteFeatures = {
            ...
            homeUpsetRate10: game.underdogTeam === "home"
                ? game.awayUpsetRate10
                : game.homeUpsetRate10,
            homeUpsetRate5: game.underdogTeam === "home"
                ? game.awayUpsetRate5
                : game.homeUpsetRate5,
            recentDogTrend10: game.underdogTeam === "home"
            ...
            homeDogAdvantage: game.underdogTeam === "home"
    ...
    for (const feature of features) {
        if (feature.key === "homeAsUnderdog") {
            ...
            console.log(`   ${feature.label.padEnd(25)}: ${winnerHomePct.toFixed(1)}% vs ${loserHomePct.toFixed(1)}% ` +
                `(${diff > 0 ? "+" : ""}${diff.toFixed(1)}pp) ${Math.abs(diff) > 5 ? "🔥" : ""}`);
            continue;
        }
        const winnerAvg = winners.reduce((sum, g) => sum + g[feature.key], 0) /
            winners.length;
        const loserAvg = losers.reduce((sum, g) => sum + g[feature.key], 0) /
            losers.length;
        ...
        const arrow = diff > 0 ? "↑" : "↓";
        console.log(`   ${feature.label.padEnd(25)}: ${winnerAvg.toFixed(3)} vs ${loserAvg.toFixed(3)} ` +
            `(${diff > 0 ? "+" : ""}${diff.toFixed(3)}) ${arrow} ${isSignificant ? "🔥" : ""}`);
    }
    // Additional insights
    console.log(chalk.bold.blue(`\n📈 ADDITIONAL INSIGHTS:\n`));
    // Upset rate buckets
    const highUpsetWinners = winners.filter((g) => (g.homeAsUnderdog ? g.homeUpsetRate10 : g.awayUpsetRate10) > 0.3).length;
    const lowUpsetWinners = winners.filter((g) => (g.homeAsUnderdog ? g.homeUpsetRate10 : g.awayUpsetRate10) <= 0.3).length;
    console.log(`   High upset history (>30%): ${highUpsetWinners}/${winners.length} ` +
        `(${((highUpsetWinners / winners.length) * 100).toFixed(1)}%)`);
    console.log(`   Low upset history (≤30%): ${lowUpsetWinners}/${winners.length} ` +
        `(${((lowUpsetWinners / winners.length) * 100).toFixed(1)}%)`);
    // Market overreaction
    const overreactedWinners = winners.filter((g) => g.marketOverreaction > 0.05).length;
    console.log(`   Market overreaction (>5%): ${overreactedWinners}/${winners.length} ` +
        `(${((overreactedWinners / winners.length) * 100).toFixed(1)}%)`);
    // Recent dog trend
    const positiveTrendWinners = winners.filter((g) => g.recentDogTrend10 > 0).length;
    console.log(`   Positive dog trend: ${positiveTrendWinners}/${winners.length} ` +
...
//# sourceMappingURL=analyze-winners.js.map