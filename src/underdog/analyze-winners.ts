...
import {
  computeUnderdogFeatures,
  filterUnderdogGames,
} from "./underdog-features.js";
...
export function analyzeWinningUnderdogs(
  ...
  if (minOdds !== undefined || maxOdds !== undefined) {
    console.log(
      chalk.dim(`   Odds Range: ${minOdds ?? 0} to ${maxOdds ?? 999}\n`),
    );
  }
  ...
  if (minOdds !== undefined || maxOdds !== undefined) {
    filteredGames = underdogGames.filter((g) => {
      ...
      `,
        ...
        .get(g.gameId) as
        | { price_home: number | null; price_away: number | null }
        | undefined;

      if (!odds) return false;

      const underdogOdds =
        g.underdogTeam === "home" ? odds.price_home : odds.price_away;
  ...
  console.log(
    `   Underdog Wins: ${winners.length} (${((winners.length / filteredGames.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `   Underdog Losses (Favorite Wins): ${losers.length} (${((losers.length / filteredGames.length) * 100).toFixed(1)}%)`,
  );
  ...
  for (const game of winners) {
    ...
    const favoriteFeatures = {
      ...
      homeUpsetRate10:
        game.underdogTeam === "home"
          ? game.awayUpsetRate10
          : game.homeUpsetRate10,
      homeUpsetRate5:
        game.underdogTeam === "home"
          ? game.awayUpsetRate5
          : game.homeUpsetRate5,
  ...
  for (const feature of features) {
    ...
    const winnerAvg =
      winners.reduce((sum, g) => sum + (g as any)[feature.key], 0) /
      winners.length;
    const loserAvg =
      losers.reduce((sum, g) => sum + (g as any)[feature.key], 0) /
      losers.length;
  ...
  // Upset rate buckets
  const highUpsetWinners = winners.filter(
    (g) => (g.homeAsUnderdog ? g.homeUpsetRate10 : g.awayUpsetRate10) > 0.3,
  ).length;
  const lowUpsetWinners = winners.filter(
    (g) => (g.homeAsUnderdog ? g.homeUpsetRate10 : g.awayUpsetRate10) <= 0.3,
  ).length;
  console.log(
  ...
  // Market overreaction
  const overreactedWinners = winners.filter(
    (g) => g.marketOverreaction > 0.05,
  ).length;
  console.log(
  ...
  // Recent dog trend
  const positiveTrendWinners = winners.filter(
    (g) => g.recentDogTrend10 > 0,
  ).length;
  console.log(
  ...
  console.log(
    `   Home underdogs: ${homeWinners}/${winners.length} (${((homeWinners / winners.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `   Away underdogs: ${awayWinners}/${winners.length} (${((awayWinners / winners.length) * 100).toFixed(1)}%)`,
  );
  ...
  const favHomeRate = losingFavorites.filter(
    (f) => f.favoriteFeatures.isFavoriteAtHome === 1,
  ).length;
  const favAwayRate = losingFavorites.length - favHomeRate;
  console.log(
    `   Home favorites who lose: ${favHomeRate}/${losingFavorites.length} (${((favHomeRate / losingFavorites.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `   Away favorites who lose: ${favAwayRate}/${losingFavorites.length} (${((favAwayRate / losingFavorites.length) * 100).toFixed(1)}%)`,
  );

  // Average losing favorite features
  const avgFavUpsetRate10 =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.homeUpsetRate10,
      0,
    ) / losingFavorites.length;
  const avgFavUpsetRate5 =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.homeUpsetRate5,
      0,
    ) / losingFavorites.length;
  const avgFavPaceDiff =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.paceDifferential,
      0,
    ) / losingFavorites.length;
  const avgFavConfDiff =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.confStrengthDiff,
      0,
    ) / losingFavorites.length;
  const avgFavMarketOverreact =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.marketOverreaction,
      0,
    ) / losingFavorites.length;
  const avgFavDogTrend =
    losingFavorites.reduce(
      (sum, f) => sum + f.favoriteFeatures.recentDogTrend10,
      0,
    ) / losingFavorites.length;

  console.log(chalk.dim(`\n   Vulnerable Favorite Profile:`));
  console.log(
    `   • Upset rate (10 games): ${avgFavUpsetRate10.toFixed(3)} ${avgFavUpsetRate10 > 0.3 ? "🔥 (high upset risk)" : ""}`,
  );
  console.log(
    `   • Pace differential: ${avgFavPaceDiff.toFixed(3)} ${avgFavPaceDiff > 0 ? "(faster)" : "(slower)"}`,
  );
  console.log(
    `   • Conference strength gap: ${avgFavConfDiff.toFixed(3)} ${Math.abs(avgFavConfDiff) < 0.05 ? "🔥 (narrow gap)" : ""}`,
  );
  console.log(
    `   • Market overreaction: ${avgFavMarketOverreact.toFixed(3)} ${avgFavMarketOverreact < -0.1 ? "🔥 (overvalued)" : ""}`,
  );
  console.log(`   • Recent underdog trend: ${avgFavDogTrend.toFixed(3)}`);

  console.log(chalk.bold.cyan(`\n💡 KEY INSIGHTS:`));
  console.log(
    `   ✓ Underdogs win ${((homeWinners / winners.length) * 100).toFixed(0)}% of the time at home vs ${((awayWinners / winners.length) * 100).toFixed(0)}% on road`,
  );
  console.log(
    `   ✓ Favorites lose ${((favHomeRate / losingFavorites.length) * 100).toFixed(0)}% at home vs ${((favAwayRate / losingFavorites.length) * 100).toFixed(0)}% on road`,
  );
  console.log(
    `   ✓ Look for: Home dogs from decent conferences with recent underdog losses`,
  );
  console.log(
    `   ✓ Fade: Favorites with narrow conference advantage and market overvaluation`,
  );
