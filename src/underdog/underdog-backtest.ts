...
function loadLatestModel(
  sport: "ncaam",
): { baseModel: UnderdogModel; marketModel: UnderdogModel } | null {
...
export async function backtestUnderdogModel(
  ...
  console.log(
    chalk.dim(`Filtered to ${underdogGames.length} underdog games\n`),
  );
  ...
  console.log(
    chalk.bold.cyan(`   ROI: ${roi >= 0 ? "+" : ""}${roi.toFixed(2)}%`),
  );
  ...
  console.log(
    chalk.dim(`   Closing Line Value: +${closingLineValue.toFixed(2)}%\n`),
  );
  ...
  for (const range of oddsRanges) {
    ...
    console.log(
      chalk.dim(
        `   ${emoji} ${range.range}: ${range.wins}/${range.bets} (${rangeWinRate}%) | ROI: ${parseFloat(rangeROI) >= 0 ? "+" : ""}${rangeROI}% | Profit: $${range.profit.toFixed(2)}`,
      ),
    );
  }
...
function calculateECE(
  ...
  const bins: Array<{ predicted: number[]; actual: number[] }> = Array.from(
    { length: numBins },
    () => ({
      predicted: [],
      actual: [],
...
export async function compareUnderdogVsMainModel(
  ...
  const underdogResults = await backtestUnderdogModel(
    sport as any,
    seasons,
    undefined,
    0.03,
  ...
  console.log(
    chalk.yellow(
      "   Note: Main model has guardrails that suppress many underdogs.",
    ),
  );
  console.log(
    chalk.yellow(
      "   This comparison shows what the main model WOULD do without guardrails.\n",
    ),
  );
  ...
  console.log(
    chalk.dim(
      "   Would show: ROI, win rate, bet count for main model on same games\n",
    ),
  );
  ...
  console.log(
    chalk.dim(
      `     ROI: ${underdogResults.roi >= 0 ? "+" : ""}${underdogResults.roi.toFixed(2)}%`,
    ),
  );
  console.log(
    chalk.dim(`     Win Rate: ${(underdogResults.winRate * 100).toFixed(1)}%`),
  );
  console.log(chalk.dim(`     Total Bets: ${underdogResults.totalBets}`));
  console.log(
    chalk.dim(`     Profit: $${underdogResults.totalProfit.toFixed(2)}\n`),
  );

  console.log(
    chalk.yellow(
      "   💡 Key Insight: Underdog model focuses ONLY on dogs (+100 or better)",
    ),
  );
  console.log(
    chalk.yellow("      while main model optimizes across all games.\n"),
  );
}
