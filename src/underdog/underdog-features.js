 ...
 */
import { computeFeatures } from "../model/features.js";
 ...
 */
function classifyUnderdogTier(marketProb) {
    if (marketProb >= 0.5)
        return null; // Not an underdog
    if (marketProb >= 0.33)
        return "moderate"; // +100 to +199 (~33-50%)
    if (marketProb >= 0.25)
        return "heavy"; // +200 to +299 (~25-33%)
 ...
 */
export function computeUnderdogFeatures(db, sport, seasons) {
    ...
    `)
        ...
        const confStrengthDiff = features.homeOppWinRate5 - features.awayOppWinRate5;
        // Recent underdog trend
        const recentDogTrend5 = computeRecentDogTrend(homeHistoryBefore, awayHistoryBefore, 5);
        const recentDogTrend10 = computeRecentDogTrend(homeHistoryBefore, awayHistoryBefore, 10);
        // Market overreaction (how much did market move vs actual performance)
        const marketOverreaction = computeMarketOverreaction(features, homeMarketProb, awayMarketProb);
 ...
 */
function buildUnderdogHistory(db, sport, seasons) {
 ...
 */
function computeUpsetRate(history, window) {
    const recentUnderdog = history.filter((h) => h.wasUnderdog).slice(-window);
    if (recentUnderdog.length === 0)
        return 0;
 ...
 */
function computeRecentDogTrend(homeHistory, awayHistory, window) {
    ...
    const weights = window === 5 ? RECENCY_WEIGHTS_5 : RECENCY_WEIGHTS_10;
    const homeTrend = homeRecentDog.length > 0
        ? weightedAvg(homeRecentDog.map((h) => h.margin), weights)
        : 0;
    const awayTrend = awayRecentDog.length > 0
        ? weightedAvg(awayRecentDog.map((h) => h.margin), weights)
        : 0;
 ...
 */
function computeMarketOverreaction(features, homeMarketProb, awayMarketProb) {
 ...
 */
export function filterUnderdogGames(features, tiers) {
    return features.filter((f) => {
        if (f.underdogTier === null)
            return false;
        if (tiers && !tiers.includes(f.underdogTier))
            return false;
        return true;
    });
}
//# sourceMappingURL=underdog-features.js.map