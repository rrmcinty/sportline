...
function buildUnderdogHistory(
  ...
  sport: string,
  seasons: number[]
): Map<number, Array<{ date: string; wasUnderdog: boolean; won: boolean; margin: number; odds: number }>> {
  ...
  const history = new Map<number, Array<{ date: string; wasUnderdog: boolean; won: boolean; margin: number; odds: number }>>();
...
function computeUpsetRate(
  ...
  const recentUnderdog = history
    .filter(h => h.wasUnderdog)
    .slice(-window);
