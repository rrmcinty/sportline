...
import { computeUnderdogFeatures, } from "./underdog-features.js";
 ...
 */
function loadLatestModel(sport) {
 ...
 */
function kelleySizing(modelProb, odds) {
    // Kelley = (bp - q) / b
 ...
 */
function matchesOptimalProfile(game) {
    ...
    if (!isHomeUnderdog)
        return false;
    let factorCount = 0;
    if (hasRecentDogLosses)
        factorCount++;
    if (hasConfStrength)
        factorCount++;
    if (hasMarketOverreaction)
        factorCount++;
 ...
 */
export async function predictUnderdogs(sport, date, minOdds = 110, maxOdds = 300, filterOptimal = false) {
  ...
  `)
    ...
    let underdogGames = gamesOnDate.filter((g) => {
        if (!g.underdogTier)
            return false;
        const underdogMarketProb = g.underdogTeam === "home" ? g.marketImpliedProb : 1 - g.marketImpliedProb;
 ...
 */
export function displayUnderdogPredictions(predictions, filterOptimal = false) {
...
//# sourceMappingURL=underdog-predict.js.map