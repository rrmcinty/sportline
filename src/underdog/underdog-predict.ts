 ...
 */
function extractUnderdogFeatures(
...
function loadLatestModel(
  ...
  baseModel: UnderdogModel;
  marketModel: UnderdogModel;
  modelDir: string;
} | null {
...
export async function predictUnderdogs(
  ...
  if (!model) {
    console.log(
      chalk.red(
        "❌ No trained model found. Run 'sportline underdog train' first.",
      ),
    );
    return [];
  }
  ...
  console.log(
    chalk.dim(`   ECE: ${model.baseModel.metrics.ece.toFixed(2)}%\n`),
  );
  ...
  let underdogGames = gamesOnDate.filter((g) => {
    ...
    const underdogMarketProb =
      g.underdogTeam === "home" ? g.marketImpliedProb : 1 - g.marketImpliedProb;
  ...
  console.log(
    chalk.dim(`Filtered to ${underdogGames.length} underdogs in odds range`),
  );
  ...
  if (filterOptimal) {
    ...
    console.log(
      chalk.dim(
        `Applied optimal profile filter: ${underdogGames.length}/${beforeFilter} games match\n`,
      ),
    );
  } else {
    console.log();
  ...
  for (const game of underdogGames) {
    ...
    const marketProb = sigmoid(
      dot(marketFeatureVec, model.marketModel.weights),
    );
    ...
    const betAmount = 10;
    const expectedValue =
      ensembleProb * betAmount * (odds / 100) - (1 - ensembleProb) * betAmount;
    ...
    if (edge > 0.1 && ensembleProb > 0.4) {
      confidence = "high";
    } else if (edge > 0.05 && ensembleProb > 0.3) {
      confidence = "medium";
    } else {
      confidence = "low";
    }
 ...
 */
export function displayUnderdogPredictions(
  predictions: UnderdogPrediction[],
  filterOptimal: boolean = false,
): void {
  ...
  for (let i = 0; i < predictions.length; i++) {
    ...
    const confidenceEmoji =
      p.confidence === "high" ? "🔥" : p.confidence === "medium" ? "⚠️" : "ℹ️";
    const tierBadge = `[${p.underdogTier.toUpperCase()}]`;

    console.log(
      chalk.bold(
        `${i + 1}. ${confidenceEmoji} ${underdogName} +${p.odds} ${tierBadge}`,
      ),
    );
    console.log(chalk.dim(`   vs ${favoriteName}`));
    console.log(
      chalk.cyan(
        `   Model: ${(p.modelProbability * 100).toFixed(1)}% | Market: ${(p.marketProbability * 100).toFixed(1)}% | Edge: +${(p.edge * 100).toFixed(1)}%`,
      ),
    );
    console.log(
      chalk.dim(
        `   Expected Value: $${p.expectedValue.toFixed(2)} per $10 bet`,
      ),
    );
    console.log(
      chalk.dim(
        `   Kelley Sizing: ${(p.kelleySizing * 100).toFixed(1)}% of bankroll`,
      ),
    );
    console.log();
  }
  ...
  const avgEdge =
    predictions.reduce((sum, p) => sum + p.edge, 0) / predictions.length;

  console.log(chalk.bold.cyan(`📊 Summary:`));
  console.log(
    chalk.dim(
      `   Total Expected Value: $${totalEV.toFixed(2)} (if betting $10 on each)`,
    ),
  );
  console.log(chalk.dim(`   Average Edge: +${(avgEdge * 100).toFixed(1)}%`));
  console.log(
    chalk.dim(
      `   High Confidence: ${predictions.filter((p) => p.confidence === "high").length}`,
    ),
  );
  console.log(
    chalk.dim(
      `   Medium Confidence: ${predictions.filter((p) => p.confidence === "medium").length}`,
    ),
  );
  console.log(
    chalk.dim(
      `   Low Confidence: ${predictions.filter((p) => p.confidence === "low").length}\n`,
    ),
  );
}
