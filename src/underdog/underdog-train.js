 ...
 */
function trainLogisticRegression(features, labels, learningRate = 0.01, iterations = 1000, lambda = 0.5, sampleWeights) {
    const numFeatures = features[0].length;
    const weights = new Array(numFeatures).fill(0);
    const n = features.length;
    const useWeights = Array.isArray(sampleWeights) && sampleWeights.length === n;
    const weightSum = useWeights
        ? sampleWeights.reduce((a, b) => a + b, 0)
        : n;
    for (let iter = 0; iter < iterations; iter++) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const w = useWeights ? sampleWeights[i] : 1;
 ...
 */
function computeRecencyWeights(dates, halfLifeDays = 120) {
    if (!dates.length)
        return [];
 ...
 */
function computeClassBalancedWeights(labels, tier) {
 ...
 */
function extractUnderdogFeatures(game, includeMarket) {
 ...
 */
function getFeatureNames(includeMarket) {
    const names = [
        "underdogWinRate5",
        "favoriteWinRate5",
        "underdogAvgMargin5",
 ...
 */
export async function trainUnderdogModel(sport, seasons, tiers) {
    ...
    const baseWeights = trainLogisticRegression(baseFeatures, baseLabels, 0.003, // learning rate (even lower)
    ...
    const marketWeights = trainLogisticRegression(marketFeatures, baseLabels, 0.003, 600, 3.0, sampleWeights);
    ...
    for (const i of valIndices) {
        ...
        if (ensembleProb > 0.5 && outcomes[i] === 1)
            correct++;
        if (ensembleProb <= 0.5 && outcomes[i] === 0)
            correct++;
 ...
 */
function calculateECE(predictions, actuals, numBins = 10) {
...
//# sourceMappingURL=underdog-train.js.map