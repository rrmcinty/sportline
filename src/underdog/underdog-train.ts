 ...
 */
function computeClassBalancedWeights(
  labels: number[],
  tier: UnderdogTier,
): number[] {
...
export async function trainUnderdogModel(
  ...
  console.log(
    chalk.dim(
      `   Approach: 50/50 ensemble (base + market-aware) with class balancing\n`,
    ),
  );
  ...
  console.log(
    chalk.dim(`Computed features for ${allFeatures.length} total games\n`),
  );
  ...
  console.log(
    chalk.dim(
      `Filtered to ${underdogGames.length} underdog games (${tiers.join(", ")})\n`,
    ),
  );
  ...
  if (underdogGames.length < 50) {
    console.log(
      chalk.red(
        `❌ Not enough underdog games (need at least 50, found ${underdogGames.length})`,
      ),
    );
    return;
  }
  ...
  const baseFeatures = trainIndices.map((i) =>
    extractUnderdogFeatures(gamesWithOutcomes[i], false),
  );
  ...
  const marketFeatures = trainIndices.map((i) =>
    extractUnderdogFeatures(gamesWithOutcomes[i], true),
  );
  ...
  console.log(
    chalk.bold.blue("Validating ensemble (50% base + 50% market-aware)...\n"),
  );
  ...
  for (const i of valIndices) {
    ...
    const underdogMarketProb =
      game.underdogTeam === "home"
        ...
        : 1 - game.marketImpliedProb;
    const odds =
      underdogMarketProb > 0
        ? Math.round((1 / underdogMarketProb - 1) * 100)
        : 100;

    if (ensembleProb > underdogMarketProb + 0.03) {
      // 3% edge threshold
      const betAmount = 10;
      if (outcomes[i] === 1) {
  ...
  console.log(
    chalk.dim(`   Underdog Win Rate: ${(winRate * 100).toFixed(1)}%`),
  );
  ...
  const modelDir = join(
    ...
    "models",
    `underdog-${sport}`,
    `underdog_${tiers.join("-")}_${seasons.join("-")}_${timestamp}`,
  );
  ...
  writeFileSync(
    join(modelDir, "base-model.json"),
    JSON.stringify(baseModel, null, 2),
  );
  writeFileSync(
    join(modelDir, "market-aware-model.json"),
  ...
  writeFileSync(
    join(modelDir, "ensemble-metadata.json"),
